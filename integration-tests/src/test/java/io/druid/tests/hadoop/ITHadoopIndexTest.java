...
public class ITHadoopIndexTest extends AbstractIndexerTest
{
  ...
  private static String BATCH_TASK = "/hadoop/batch_hadoop_indexer.json";
  private static String BATCH_QUERIES_RESOURCE = "/hadoop/batch_hadoop_queries.json";
  private static String BATCH_DATASOURCE = "batchHadoop";
  ...
  private void loadData(String hadoopDir)
  {
    ...
    try {
      ...
      // use longer timeout than waitUntilTaskCompletes() does;
      // the grid might be slow
      RetryUtil.retryUntil(
          new Callable<Boolean>()
          {
            ...
            {
              TaskStatus.Status status = indexer.getTaskStatus(taskID);
  ...
  public void afterClass()
  {
      if (dataLoaded)
      {
	     ...
	     unloadAndKillData(BATCH_DATASOURCE);
              ...
              LOG.warn ("exception while removing segments: " + e.getMessage());
         }
      }
  }
}
