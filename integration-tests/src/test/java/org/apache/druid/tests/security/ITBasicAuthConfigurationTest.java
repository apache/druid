...
public class ITBasicAuthConfigurationTest
{
  ...
  private void testAvaticaAuthFailure(String url) throws Exception
  {
    ...
    try {
      ...
      Assert.assertEquals(
          ase.getErrorMessage(),
          "Error while executing SQL \"SELECT * FROM INFORMATION_SCHEMA.COLUMNS\": Remote driver error: BasicSecurityAuthenticationException: User authentication failed username[admin]."
      );
      return;
    }
