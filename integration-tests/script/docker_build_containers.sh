...
then
  ...
  8)
    ...
    docker build -t druid/cluster --build-arg JDK_VERSION=8-slim $SHARED_DIR/docker
    ;;
  11)
    echo "Build druid-cluster with Java 11"
    docker build -t druid/cluster --build-arg JDK_VERSION=11-slim $SHARED_DIR/docker
    ;;
  *)
