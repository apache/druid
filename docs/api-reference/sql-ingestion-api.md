...
| `multiStageQuery.payload.status.segmentLoadWaiterStatus` | Segment loading waiter container. Not present till the segments have been published |
| `multiStageQuery.payload.status.segmentLoadWaiterStatus.state` | RUNNING, SUCCESS, FAILED or TIMED_OUT |
| `multiStageQuery.payload.status.segmentLoadWaiterStatus.startTime` | Start time of the awaiter |
| `multiStageQuery.payload.status.segmentLoadWaiterStatus.duration` | The number of milliseconds that the awaiter has been running |
