...
| `multiStageQuery.payload.status.segmentLoadStatus.hotSegments` | The number of segments which are marked as hot based on the load rules. These segments will be loaded and served by a historical. |
| `multiStageQuery.payload.status.segmentLoadStatus.coldSegments` | The number of segments which are marked as cold based on the load rules. These segments will not be loaded on any historical. |
