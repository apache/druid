...
| tryParseNumbers| Boolean| If this is set, the task will attempt to parse numeric strings into long or double data type, in that order. If the value cannot be parsed as a number, it is retained as a string. | no (default = false) |
