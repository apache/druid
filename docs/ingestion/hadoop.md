...
|maxSegmentIntervalsPermitted|The limit on the number of segment intervals that a single indexing job can cover. If the limit is hit, the job will exit before starting the index building phase of the task. This config can help prevent an end user from starting a task that ingests more intervals than an operator wants to allow. This config tends to come in handy for multi-tenant clusters where many users ingest their own data. The default value is set as such that no job will hit this limit unless the config is explicitly added to the tuning config. Only applies when using hashed partitionsSpec with `maxRowsPerSegment` or when using single_dim partitionsSpec.|Integer.MAX_VALUE|
|maxAggregateSegmentsPermitted|The limit on the aggregate number of segments that a single indexing job can create. It is a compliment to `maxSegmentIntervalsPermitted` in the sense that it expands to cover sharding within an interval in addition to the number of intervals. If this limit is hit, the job will exit before starting the indexing building phase of the task. This config can help prevent an end user from starting a task that ingests more segments in one job than an operator wants to allow. This config tends to come in handy for multi-tenant clusters where many users ingest their own data. The default value is set as such that no job will hit this limit unless the config is explicitly added to the tuning config. Only applies when using hashed partitionsSpec with `maxRowsPerSegment` or when using single_dim partitionsSpec.|Integer.MAX_VALUE|
