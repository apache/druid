...
|maxSegmentsIngested|Only compatible with `hashed` `partitionsSpec`. The maximum number of segments allowed to be ingested by the task. If violated, the job will exit early as failed. Only applies if task must determine partitions at runtime.|`Integer.MAX_VALUE`|
