...
|complexMetricCompression|Compression format for complex type metric columns. Options are `lz4`, `lzf`, `zstd`, `uncompressed`. Options other than `uncompressed` not compatible with Druid versions older than 31, and only applies to complex metrics which do not have their own specialized column formats.|`uncompressed`|
