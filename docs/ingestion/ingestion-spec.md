...
| safeInput | Whether to enable time-check on intervals, to avoid cases where input values may be wrong, or unwanted. For example, we can tell Druid to enable the restricting function to filter out cases where the year is wrongly typed as "20240" instead of "2024". <br /> <br />Any intervals created outside the range of `safeStart` and `safeEnd` will cause the program to fold. | `false` |
| safeStart | Sets the start DateTime for the safeInput time-check functionality. <br /><br />Specify DateTime values using ISO8601 format. For example, `"2024"` If you omit the time, the time defaults to `1000-01-01T00:00:00+00:00`. | `null` |
| safeEnd | Sets the end DateTime for the safeInput time-check functionality.<br /><br />Specify DateTime values using ISO8601 format. For example, `"2024-07-31"`. If you omit the time, the time defaults to one year after the current timestamp. | `null` |
...
The following example shows a `tuningConfig` object that sets all the shared common properties to their defaults:
