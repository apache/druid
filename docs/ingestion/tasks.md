...
|`storeCompactionState`|Whether the task should store the compaction state of created segments in its metadata. If set, the segments created by the task fill `lastCompactionState` in the metadata. In most cases, you should not need to set this parameter as it is set automatically on compaction tasks. |true for compaction tasks, false for other task types|
|`storeEmptyColumns`|Whether to store empty columns during ingestion. If set to true, Druid stores every column specified in the [`dimensionsSpec`](ingestion-spec.md#dimensionsspec). <br/><br/>If set to false, Druid SQL queries referencing empty columns will fail. If you intend to leave `storeEmptyColumns` disabled, you should either ingest dummy data for empty columns or else not query on empty columns.<br/><br/>When set in the task context, `storeEmptyColumns` overrides the system property [`druid.indexer.task.storeEmptyColumns`](../configuration/index.md#additional-peon-configuration).|true|
