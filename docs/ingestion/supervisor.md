...
|`replicas`|Integer|The number of replica sets, where 1 is a single set of tasks (no replication). Druid always assigns replicate tasks to different workers to provide resiliency against process failure. See `serverPriorityToReplicas` to control server priorities for task replicas.|No|1|
...
|`serverPriorityToReplicas`|Object (`Map<Integer, Integer>`)|Map of server priority to the number of replicas per priority. When set, each task replica is assigned a server priority that corresponds to `druid.server.priority` on the Peon process to enable query isolation for mixed workloads by [query routing strategies](../configuration/index.md#query-routing)|No|null|
