...
the process for updating old values should be slower in Druid. However, the requirements in Kudu for maintaining extra head space to store 
...
fast in Druid, whereas updates of older data is slower. This is by design as the data Druid is good far is typically event data, 
...
Druid also has the capability of supporting integration with multiple frameworks, either through the broker layer, or replacing it entirely.
