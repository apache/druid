...
The two core parser types are `string` and `map`, though a handful of extensions provide additional map based types. String typed parsers operate on text based inputs that can be split into individual records by newlines. Map and map-like parsers provided by some extensions rely on an external mechanism, such as a [firehose](./firehose.html) or [Hadoop](./hadoop.html) input format implementation to split data into individual rows and populate data into a Java `Map` type. If `type` is not included, the parser defaults to `string`. For additional data formats, please see our [extensions list](../development/extensions.html).
