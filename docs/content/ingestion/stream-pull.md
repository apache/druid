...
|outputMediumFactory|String|Output Medium to use when creating segments. See [Indexing Service Configuration](../configuration/indexing-service.html) page, "OutputMediumFactory" for explanation and available options.|no (not specified by default, the value from `druid.peon.defaultOutputMediumFactory` is used)|
