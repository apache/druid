...
|`druid.coordinator.merge.on`|Deprecated. Use [compaction](../operations/api-reference.html#compaction-configuration) instead. Boolean flag for whether or not the coordinator should try and merge small segments into a more optimal segment size.|false|
