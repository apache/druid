...
|`druid.coordinator.balancer.strategy`|Specify the type of balancing strategy that the coordinator should use to distribute segments among the historicals. Use diskNormalizedCostBalancer to distribute segments among nodes so that the disks fill up uniformly.|costBalancer|
