...
|`druid.coordinator.kill.durationToRetain`| Do not kill segments with data interval in last `durationToRetain`, must be greater or equal to 0. Only applies and MUST be specified if kill is turned on. Note that default value is invalid.|PT-1S (-1 seconds)|
|`druid.coordinator.kill.durationToRetainUnusedSegments`| Do not kill segments disabled in last `durationToRetainUnusedSegments`, must be greater or equal to 0. Only applies and MUST be specified if kill is turned on. Note that default value is invalid.|PT-1S (-1 seconds)|
