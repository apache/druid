...
|`maxIntermediateRows`|varies|Maximum number of intermediate rows while processing single segment for groupBy or topN query. Default value used can be specified by updating `druid.query.groupBy.maxIntermediateRows` for groupBy query and `druid.query.topN.maxIntermediateRows` for topN query in druid configuration at broker and historical nodes. At query time you can only lower the value.|
