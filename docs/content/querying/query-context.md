...
|maxScatterGatherBytes| Long.MAX_VALUE | At Broker, this limits total number of bytes gathered from downstream nodes such as historicals and realtime indexers. If Broker is under heavy load and not consuming the data fast enough coming from downstream nodes then it gets stored in memory and may lead to OOMs. This setting provides a workaround to limit per query max memory utilization for storing data received from downstream nodes.|
