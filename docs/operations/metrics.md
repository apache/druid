...
|`segment/serverview/sync/success`|Sync status of the broker server view with a segment server such as historicals and peons. Emitted only when [HTTP-based server view](../configuration/index.md#segment-management) is enabled.|`server`|1 for fully synced servers, 0 otherwise|
|`segment/serverview/sync/unstableTime`|Time in milliseconds for which the broker server view has been unable to sync with a segment server. Emitted for unsynced servers only when [HTTP-based server view](../configuration/index.md#segment-management) is enabled.|`server`|Not emitted for synced servers.|
...
|`segment/serverview/sync/success`|Sync status of the coordinator server view with a segment server such as historicals and peons. Emitted only when [HTTP-based server view](../configuration/index.md#segment-management) is enabled.|`server`|1 for fully synced servers, 0 otherwise|
|`segment/serverview/sync/unstableTime`|Time in milliseconds for which the coordinator server view has been unable to sync with a segment server. Emitted for unsynced servers only when [HTTP-based server view](../configuration/index.md#segment-management) is enabled.|`server`|Not emitted for synced servers.|
