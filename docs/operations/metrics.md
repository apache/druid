...
|`supervisor/count`|Count of active supervisors. Each supervisor emits 1, tagged with its state (`RUNNING`, `SUSPENDED`, `UNHEALTHY_SUPERVISOR`, `UNHEALTHY_TASKS`, etc.). This metric is available only if the `SupervisorStatsMonitor` module is included.|`supervisorId`, `type`, `state`|1 per supervisor; aggregate by state for totals|
