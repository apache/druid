...
|`guild/replicationMinimumViolation/count`|Total number of used segments who reside on <= 1 guild. This metric is only available if `druid.coordinator.guildReplication.on=true` in the coordinator runtime properties, and `emitGuildReplicationMetrics=true` in the coordinator dynamic config. Enabling it requires the coordinator to iterate all used segments to compute the metric, which comes with a compute cost.|datasource.|close to 0 if all segments have replicaiton factor > 1|
