...
|`segment/used/deepStorageOnly/count`|Number of used segments present only in deep storage.|`dataSource`||
|`segment/schemaCache/deepStorageOnly/refresh/count`|Number of deep storage only segments with cached schema.|`dataSource`||
|`segment/schemaCache/deepStorageOnly/refresh/time`|Time taken in milliseconds to process deep storage only segment schema.||Under a minute|
