...
|`segment/skipCompact/bytes`|Total bytes of this datasource that are skipped (not compacted in this run) by the auto compaction. Emitted once per skip `reason`; sum across all reasons for the datasource total. The `category` dimension groups reasons into `OUT_OF_SCOPE` (never considered for compaction), `TRANSIENT` (expected to be picked up in a later run), `DEFERRED` (needs compaction, but a threshold or policy you configured is holding it back) and `UNSUPPORTED` (cannot be compacted as it currently stands). Only the last two count against a datasource being fully compacted. Alert on `category` rather than on individual reasons, since more reasons may be added in future releases.|`dataSource`, `reason`, `category`|Varies|
|`segment/skipCompact/count`|Total number of segments of this datasource that are skipped (not compacted in this run) by the auto compaction. Emitted once per skip `reason`.|`dataSource`, `reason`, `category`|Varies|
|`interval/skipCompact/count`|Total number of intervals of this datasource that are skipped (not compacted in this run) by the auto compaction. Emitted once per skip `reason`.|`dataSource`, `reason`, `category`|Varies|
