...
* `buildRevision`: the git commit hash of the build that produced the service JAR. Useful for verifying that all nodes in a cluster are running the intended revision during rolling deployments. Empty string when running outside a packaged JAR (e.g., during `mvn test`).
