...
|`segment/skipCompact/bytes`|Total bytes of this datasource that are skipped (not compacted in this run) by the auto compaction. Emitted once per skip `reason`; sum across all reasons for the datasource total. The `category` dimension groups reasons into `OUT_OF_SCOPE` (deliberately excluded by the compaction config), `TRANSIENT` (expected to be picked up in a later run), `DEFERRED` (does not match the config until the config or policy is changed) and `UNSUPPORTED` (cannot be compacted as it currently stands). Alert on `category` rather than on individual reasons, since more reasons may be added in future releases.|`dataSource`, `reason`, `category`|Varies|
