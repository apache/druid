...
|`druid.sql.planner.forceExpressionVirtualColumns`|If true, the query plan does not contain any specialized virtual columns. Instead, specialized virtual columns (such as those used to plan `MV_FILTER_ONLY` and `MV_FILTER_NONE` SQL functions) are planned as "expression"-type virtual columns. If false, such virtual columns are planned in their respective specialized forms.|false|
