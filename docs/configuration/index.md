...
|`druid.sql.sys.segments.stringCacheSize`|When computing a query against the `sys.segments` table, the broker uses Caffeine-based in-memory caches to temporarily store string representation of Java objects, so that it can avoid repetitive calls of expensive serialization for the same object during the query. You may want to raise this config if your segments have diverse sets of dimensions. Human-readable format is supported, see [here](human-readable-byte.md).|1MiB|
