...
|`druid.indexer.runner.taskSlotRatiosPerWorker`|Map from task type to the ratio of task slots that the given task type can occupy on a single worker. Each value must be in the range [0, 1]. A value of 0 for a task type means that it can take up no task slot on a worker (i.e. tasks of this type would never run). A value of 1 means that the task type may take up all task slots of a worker if available. For `index_parallel` tasks, the `parallelIndexTaskSlotRatio` overrides the value specified in this map.|Empty map i.e. no limit on task slots for any task type|
