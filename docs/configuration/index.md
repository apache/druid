...
|`druid.coordinator.loadqueuepeon.http.batchSize`|Number of segment load/drop requests to batch in one HTTP request. Note that it must be smaller than or equal to the `druid.segmentCache.numLoadingThreads` config on Historical service. If the value is not provided, the coordinator automatically dynamically configures the value to the `numLoadingThreads` available on the Historical. | `druid.segmentCache.numLoadingThreads` |
