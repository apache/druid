...
|`druid.broker.segment.watchRealtimeNodes`|Boolean|Broker watches segment announcements from processes serving segments to build cache of which process is serving which segments. This configuration allows the exclusion of segments being served by realtime processes. By default, Broker would consider both realtime and historical processes. This can be used to configure brokers in partitions so that they are only queryable for Historical tiers or both Historical tiers and realtime processes.|true|
