...
|`druid.processing.parallelPoolInit`|Allows all merge/processing pools to be allocated in parallel on process launch. This significantly speeds up task launch times.|`false`|
...
|`druid.processing.fifo`|If the processing queue should treat tasks of equal priority in a FIFO manner|`true`|
|`druid.processing.tmpDir`|Path where temporary files created while processing a query should be stored. If specified, this configuration takes priority over the default `java.io.tmpdir` path.|path represented by `java.io.tmpdir`|
|`druid.processing.parallelPoolInit`|Allows all merge/processing pools to be allocated in parallel on process launch. This significantly speeds up node launch times.|`false`|
