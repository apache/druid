...
|`druid.indexer.task.batchProcessingMode`| These flags help avoid "out of memory" faults in the segment creation phase of batch ingestion. It can take one of the following string values, each one with more aggressive memory optimizations: {`LEGACY`,`CLOSED_SEGMENTS`, `CLOSED_SEGMENTS_SINKS`}. If value is `LEGACY` it will use the original code path for the create segments phase of batch ingestion. If value is `CLOSED_SEGMENTS` native batch ingestion will use a new, recommended, code path with memory optimized code for the segment creation phase.  If value is `CLOSED_SEGMENS_SINKS` the code path is a more aggressive memory optimized code; you may try this mode if you are getting OOMs in the segment phase creation of batch ingestion in both of the previous modes. This does not apply to streaming ingestion, just to batch. If you misspell the flag name the system will use the default. |`CLOSED_SEGMENTS`|
