...
|`druid.coordinator.kill.durationToRetain`|Only applies and if kill is turned on. Do not kill unused segments whose interval end is < `now - durationToRetain`. durationToRetain can be negative, which would result in `now - durationToRetain` to be in the future. The default, `PT0s`, means that the coordinator will delete any unused segments whose interval end is less than or equal to the instant the coordinator runs the kill logic.|`PT0s`|
|`druid.coordinator.kill.ignoreDurationToRetain`|A way to override `durationToRetain` and tell the coordinator that you do not care about the end interval of unused segments when it comes to killing them. If true, the coordinator thinks any and all unused segments are eligible to be killed.|false|
