...
|`druid.sql.avatica.maxRowsPerFrame`|Maximum number of rows to return in a single JDBC frame. Setting this property to -1 indicates that no row limit should be applied. Clients can optionally specify a row limit in their requests; if a client specifies a row limit, the lesser value of the client-provided limit and `maxRowsPerFrame` will be used. If `maxRowsPerFrame` is smaller than `minRowsPerFrame`, then frame size will become fixed. For handling larger result sets, consider increasing this value to reduce the number of frames required to transfer the result set.|5,000|
|`druid.sql.avatica.minRowsPerFrame`|Minimum number of rows to return in a single JDBC frame. This property must be set to a value greater than 0. If a client specifies a smaller fetch size in their requests, this limit will be used instead. If `maxRowsPerFrame` is set to a value lower than `minRowsPerFrame`, the minimum value of the two will be used. For handling larger result sets, consider increasing this value to reduce the number of frames required to transfer the result set.|100|
