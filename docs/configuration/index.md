...
|`coordinatingVersions`|List of Druid node versions for which the Coordinator enforces per-version Historical replication and handoff. When set, load/drop rules are applied independently to each listed version that has active servers in a tier, so each version receives the rule's required replicas. Servers outside the listed versions are not assigned new replicas and may have surplus replicas dropped. Empty disables per-version coordination.|none|
...
|`druid.broker.segment.watchedVersions`|List of strings|Restricts the Broker's segment view to data servers whose announced Druid node `version` matches one of the listed values. Realtime servers (peons and indexers) bypass this filter unless `druid.broker.segment.strictRealtimeVersionFilter` is true. Empty means no filtering.|none|
|`druid.broker.segment.strictRealtimeVersionFilter`|Boolean|When true, `druid.broker.segment.watchedVersions` also applies to realtime servers (peons, indexers). Default false: realtime servers are always watched regardless of version so Brokers can query in-flight realtime data during a rollover.|false|
...
|`druid.router.routableVersions`|If non-empty, the Router only routes queries to Brokers whose announced Druid node `version` is in this set. This filters Broker nodes within the service selected by the Router strategy; it does not change which Broker service a strategy selects. When a query's serviceName has no matching Broker after filtering, the selector returns no server and the request fails with a clear error. Empty means no filtering.|none|
