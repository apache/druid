...
|`druid.segmentCache.lazyLoadOnStart`|Whether or not to load segment columns metadata lazily during historical startup. Set this value to true will have a definite improvement during historical startup with HDD(may be 20X faster). This features is unnecessary for historical with SSD, because it is fast enough.|false|
