...
|`druid.coordinator.kill.durationToRetain`|Only applies if you set `druid.coordinator.kill.on` to `true`. Note that the default value (null) is invalid unless `druid.coordinator.kill.durationToRetain` is `true`. If `druid.coordinator.kill.durationToRetain` is false, you must provide an explicit ISO8601 period. Druid will not kill unused segments whose interval end date is beyond `now - durationToRetain`. durationToRetain can be a negative ISO8601 period, which would result in `now - durationToRetain` to be in the future.|`null`|
