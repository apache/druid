...
|`druid.sql.planner.metadataColumnTypeMergePolicy`|Defines how column types will be chosen when faced with differences between segments when computing the SQL schema. Options are specified as a JSON object, with valid choices of `{"type":"newestFirst"}` or `{"type":"leastRestrictive"}`. With `newestFirst`, the column type in most recent time chunks defines the type for the schema. For `leastRestrictive`, Druid will automatically widen the type computed for the schema to a type which data across all segments can be converted into, however planned schema migrations can only take effect once all segments have been reingested to the new schema. |`{"type":"newestFirst"}`| 
