...
|`druid.sql.planner.maxNumericInFilters`|If set to a value between 1 and 10,000, Druid will allow numeric values specified for IN part of the query not to exceed this user defined values and queries with more than 10,000 numeric IN filters will not be run by Druid unless they are cast to String. This can be overridden to a value between 1 and 10,000 through the query context.|`10000`|
