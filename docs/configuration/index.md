...
|`druid.coordinator.period.primaryReplicantLoaderPeriod`|Only used if `druid.coordinator.loadPrimaryReplicantSeparately=true`. How often the coordinator will execute its job to load primary replicas for used segments. This is the delay after completion of the previous cycle. The N + 1th cycle will start the specified period of time after the Nth cycle completes.|PT60S|
