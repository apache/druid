...
|`druid.indexer.task.useLineageBasedSegmentAllocation`|If true, [Parallel task](../ingestion/native-batch.md#parallel-task) will use the lineage-based segment allocation algorithm for dynamic partitioning that guarnatees the idempotency of the segment allocation protocol.<br/><br/>This option is provided to support rolling upgrade to Druid 0.22.0, especially when you replace idle nodes with a newer version one at a time. If you are using native batch ingestion with dynamic partitioning and `maxNumConcurrentSubTasks` > 1, you can set this option to false to not break running ingestion jobs during rolling upgrade. You can ignore this option if you use the in-place rolling upgrade. Once you upgrade to 0.22 or higher, this option must be set to true.|true|
