...
|`druid.broker.segment.unavailableCheckPeriod`|ISO 8601 period|How often the Broker asks the Coordinator about the segments it has no server for. Also governs how quickly a segment that has since been marked unused stops being reported.|`PT5S`|
|`druid.broker.segment.maxUnavailableSegments`|Integer|Cap on how many segments with no server the Broker tracks at once. Beyond this it stops tracking and reverts to dropping them from the timeline, so that a large-scale outage such as a whole tier restarting cannot grow the tracked set without bound.|`1000000`|
