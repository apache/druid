...
|`druid.indexer.runner.taskSlotLimits`|A map where each key is a task type (`String`), and the corresponding value represents the absolute limit on the number of task slots that tasks of this type can occupy on a worker. The value is an `Integer` that is greater than or equal to 0. For example, a value of 5 means that tasks of this type can occupy up to 5 task slots on a worker. If a task is of the parallel index type, this absolute limit will be considered when evaluating task slot allocation, alongside any defined ratios. The effective limit for a task type will be the smaller of the absolute limit and the limit derived from the corresponding ratio. taskSlotLimits = {"index_parallel": 3, "query_controller": 5}. In this example, parallel indexing tasks can occupy up to 3 task slots, and query controllers can occupy up to 5 task slots.|Empty map|
|`druid.indexer.runner.taskSlotRatios`|A map where each key is a task type (`String`), and the corresponding value is a `Double` which should be in the range [0, 1], representing the ratio of available task slots that tasks of this type can occupy. This ratio defines the proportion of total task slots a task type can use, calculated as `ratio * totalSlots`. If a task is of the parallel index type, this ratio will be evaluated in conjunction with any defined absolute limits to ensure optimal resource allocation. The effective limit for a task type will be the smaller of the absolute limit (from {@link #getTaskSlotLimits()}) and the limit derived from this ratio. taskSlotRatios = {"index_parallel": 0.5, "query_controller": 0.25}. In this example, parallel indexing tasks can occupy up to 50% of the total task slots, and query controllers can occupy up to 25% of the total task slots.|Empty map|
