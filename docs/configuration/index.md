...
|`druid.sql.planner.useDefaultExpressionExplain`|If true, multi-value expressions that use `MV_FILTER_ONLY` or `MV_FILTER_NONE` are planned as default expression-based virtual columns. If false, these expressions are planned as "mv-filtered" virtual columns. This parameter is honored only if `useNativeQueryExplain` is true.|false|
