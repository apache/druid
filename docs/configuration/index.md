...
|`druid.coordinator.kill.on`|Boolean flag for whether to submit a kill task for unused segments, permanently deleting them from the metadata store and deep storage. When `druid.coordinator.kill.on` is true: <ul><li>Segments become eligible for permanent deletion once their data intervals are older than `druid.coordinator.kill.durationToRetain`  relative to the current time. If a segment's data interval exceeds this threshold at the time it is marked unused, it becomes eligible for immediate deletion.</li><li>If `killDataSourceWhitelist` specifies data sources, the Coordinator issues kill tasks only for those data sources based on the specified period.</li><li>If `killDataSourceWhitelist` is empty, the Coordinator issues kill tasks for all data sources.</li></ul>|false|
