...
|`druid.coordinator.kill.maxSegments`|Kill at most n unused segments per kill task submission. This only applies if you set `druid.coordinator.kill.on` to `true` and must be greater than 0 then.|100|
