...
|`druid.sql.planner.maxNumericInFilters`|If set to a value between 1 and 100, Druid will allow numeric values on string columns specified for IN clause of the query not to exceed this user defined values and queries with more than 100 numeric IN filters will not be run by Druid unless they are cast to String. This can be changed to a value between 1 and 100 through the query context.|`100`|
