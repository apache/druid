...
| `druid.emitter.prometheus.flushPeriod`        | If strategy is `pushgateway`, metrics are emitted every `flushPeriod` seconds. If strategy is `exporter`, this configures the metric TTL such that if the metric value is not updated within `flushPeriod` seconds then it will stop being emitted. It is recommended to set this to at least 3 * scrape_interval.                                                                                                                                 | Required if `pushgateway` strategy is used, optional otherwise.        | 15 seconds for `pushgateway` strategy. None for `exporter` strategy. |
