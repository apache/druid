...
| `druid.emitter.prometheus.flushPeriod`        | If strategy is `pushgateway`, emits metrics every `flushPeriod` seconds. If strategy is `exporter`, configures the metric ttl such that if the metric value is not updated within the `flushPeriod` then it will stop being emitted.                                                                                                                                   | Required if `pushgateway` strategy is used, optional otherwise.        | 15 seconds for `pushgateway` strategy. None for `exporter` strategy. |
