...
| `druid.storage.zip` | Whether segments are written as directories of blobs (`false`) or as a single `index.zip` blob (`true`). Writing segments unzipped requires permission to delete blobs under the segment path, so that a push replaces the segment already there. | `true` |
