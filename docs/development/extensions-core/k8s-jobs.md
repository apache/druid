...
| `druid.indexer.runner.logSaveTimeout` | `Duration` | The peon executing the ingestion task makes a best effort to persist the pod logs from `k8s` to persistent task log storage. A timeout for this log persist is set to avoid `k8s` connection issues causing the peon to hang indefinitely if issues occur during the persisting of said logs. | `PT300S` | NO |
