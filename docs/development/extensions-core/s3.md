...
|`druid.storage.zip`|`true`, `false`|Whether segments in `s3` are written as directories (`false`) or zip files (`true`). Writing segments as directories requires `s3:DeleteObject` under the segment path, so that a push replaces the segment already there.|`true`|
