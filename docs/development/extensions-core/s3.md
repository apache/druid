...
To configure the extension to read objects from S3, supply S3 [connection information](#configuration) in configuration.
...
You can override the default credentials provider chain for connecting to the source bucket. Specify an access key and secret key using [Properties Object](../../ingestion/native-batch.md#s3-input-source) parameters in the ingestion specification.
...
The order of configuration parameters is important because it defines the precedence of authentication methods. If you are trying to use Instance profile information, do not set `druid.s3.accessKey` and `druid.s3.secretKey` in your Druid runtime.properties.
