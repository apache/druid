...
|`druid.s3.enableLegacyMd5`|Sends `Content-MD5` request checksums instead of the `CRC32` checksums that AWS SDK v2 calculates by default since version 2.30.0. Enable this only for S3-compatible storage that rejects the newer checksum headers. See [AWS document](https://docs.aws.amazon.com/sdk-for-java/latest/developer-guide/s3-checksums.html) for details.|false|
