...
| `enableInMemoryBitmap`            | Boolean        | If true, enable in memory bitmap for applicable dimensions during real time writes before persistence to create a columnar index kicks in. When combined with `useInMemoryBitmapInQuery` set to true in query context, queries can leverage the bitmaps to avoid a full scan for this stage of data.                                                                                                                                                                                                                                                                                                                                                | no, default == false                                                                                         |
