...
> In addition, users could set `isolation.level` false in `consumerProperties`, if don't need Druid to consume transactionally or need Druid to consume older versions of Kafka.
