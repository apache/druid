...
- `keyFormat`: Supply an input format to parse the key. Only the first value will be used.
  ...
  ```
  Note that in case of the `tsv` format (also `csv` and `regex`) you need to provide a `columns` array to make a valid input format but only the first one will be used and it's name will be ignored in favor of `keyColumnName`.
