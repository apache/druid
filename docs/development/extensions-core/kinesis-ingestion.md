...
|`skipIgnorableShards`|Boolean|Kinesis ingestion considers all shards that were not previously closed and empty for task group distribution but current shards in such a state are not used during task creation. This may lead to an uneven distribution of "useful" shards to tasks when resharding has occurred recently. This flag is meant to ignore closed and empty shards during task group assignment itself. Please note that despite using caching, this may lead to increased processing time when many such shards are discovered for the first time (Eg: supervisor startup).|no (default == false)|
