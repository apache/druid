...
|`skipIgnorableShards`|Boolean| Useful to skip certain shards created due to resharding for a more even work assignment to tasks. Please note that this may increase the number of kinesis calls made and have an initial delay in supervisor startup.|no (default == false)|
