...
| `multiStageQuery.payload.status.segmentLoadAwaiterStatus` | Segment loading awaiter container. Not present till the segments have been published |
| `multiStageQuery.payload.status.segmentLoadAwaiterStatus.state` | RUNNING, SUCCESS, FAILED or TIMED_OUT |
| `multiStageQuery.payload.status.segmentLoadAwaiterStatus.startTime` | Start time of the awaiter |
| `multiStageQuery.payload.status.segmentLoadAwaiterStatus.duration` | The number of milliseconds that the awaiter has been running |
| `multiStageQuery.payload.status.segmentLoadAwaiterStatus.totalSegments` | The total number of segments generated by the job. This includes tombstone segments (if any). |
| `multiStageQuery.payload.status.segmentLoadAwaiterStatus.segmentsLeft` | The total number of segments remaining to be loaded. |
