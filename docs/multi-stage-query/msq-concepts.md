...
It may be necessary to override one or more memory-related parameters if you run into one of the [known issues around memory usage](./msq-release-notes.md#memory-usage).
...
|  DurableStorageConfiguration  | Durable storage mode activation failed due to a misconfiguration. For configuration instructions, see [Durable storage for shuffle mesh](./msq-durable-storage.md) for instructions on configuration.  |  |
