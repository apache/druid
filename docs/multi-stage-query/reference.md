...
| `druid.export.storage.google.allowedExportPaths` | Yes      | An array of GS prefixes that are whitelisted as export destinations. Export queries fail if the export destination does not match any of the configured prefixes. Example: `[\"gs://bucket1/export/\", \"gs://bucket2/export/\"]`    | n/a |
...
| `druid.export.storage.gooel.chunkSize`           | No       | Defines the size of each chunk to temporarily store in `tempDir`. The chunk size must be between 5 MiB and 5 GiB. A large chunk size reduces the API calls to GS, however it requires more disk space to store the temporary chunks. | 100MiB |
