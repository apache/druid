...
| <a name="error_InsertCannotAllocateSegment">`InsertAllocatedIncorrectSegment`</a> | The controller task could not allocate a new segment ID of the specified granularity due to conflict with existing segments or pending segments. <br /> <br /> This happens when a coarser segment is already overlapping the interval for which the allocation was requestd. Either use a REPLACE to overwrite over the existing overlapping segment or re-run INSERT with the pre-existing segment granularity in order to append to the interval| `dataSource`<br /> <br />`requestedInterval`: The interval for the attempted new segment allocation. <br /><br />`allocatedInterval` The interval allocated for the requested segment |
