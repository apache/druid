...
Note that INSERT queries cannot perform dimension-based segment pruning. You must use REPLACE for those. For more information, see [Overwrite data with REPLACE](#overwrite-data-with-replace) and [Clustering](#clustering).
...
For more information about CLUSTERED BY, see [Clustering](#clustering).
