...
|SMALLINT|LONG|`0`||
|INTEGER|LONG|`0`||
|BIGINT|LONG|`0`|Druid LONG columns (except `__time`) are reported as BIGINT. `LONG` is also accepted as an alias for `BIGINT` in `CAST` expressions, for example `CAST(x AS LONG)`, matching the name Druid's native engine uses for this type.|
