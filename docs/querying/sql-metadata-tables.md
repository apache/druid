...
|build_revision|VARCHAR|Git commit ID of the build that produced the server JAR. Empty string when the server is running outside a packaged JAR (e.g., during `mvn test`)|
