...
|offset|Skip this many rows when returning results. When combined with "limit", this can be used to implement pagination. Note that the skipped rows will still need to be generated internally and then discarded, meaning that high offsets can cause queries to use additional resources.|no|
