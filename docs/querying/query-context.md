...
|useSegmentMergedResultCache         | `true`                      | Flag indicating whether to leverage the segment merged result cache for this query. When set to false, it disables reading from the query cache for this query. When set to true, Druid uses `druid.broker.cache.useSegmentMergedResultCache` to determine whether or not to read from the segment merged result cache |
|populateSegmentMergedResultCache    | `true`                      | Flag indicating whether to save the results of the query to the segment merged result cache. Primarily used for debugging. When set to false, it disables saving the results of this query to the query cache. When set to true, Druid uses `druid.broker.cache.populateSegmentMergedResultCache` to determine whether or not to save the results of this query to the segment merged result cache |
