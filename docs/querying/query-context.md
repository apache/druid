...
|debug| `false` | Flag indicating whether to enable debugging output for the query. When false, Druid does not produce additional logs. The logs correspond to your logging level. When `true`, Druid produces the following addition logs :<br />- Log the stack trace of the exception (if any) produced by the query |
