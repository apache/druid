...
|maxNumericInFilters|`100`|For columns in String format containing numeric values, Druid will allow numeric values specified for IN clause of the query over the String columns not to exceed this user defined value. Queries with more than 100 numeric IN filters wont be executed. Users are advised to cast their values to String if they have more than 100 IN filters in their query. For example, instead of IN(1,2,3,...) specify as IN('1','2','3',...)|
