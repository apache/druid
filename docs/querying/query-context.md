...
|maxNumericInFilters|`-1`|Max limit for the amount of numeric values that can be compared for a string type dimension when a SQL WHERE clause translates to an [IN filter](../querying/filters.md#in-filter). Set this property to a smaller value to prevent prohibitively slow queries when filtering on numbers in string type dimensions. Setting the optimal limit requires some trial and error; we recommend starting with 100. For more than 100 numbers, prevent slow IN filters by casting the the compared values as strings. For example, `WHERE someString IN ('123', '456')`. By default, Druid does not restrict the amount of numbers in an IN filter.|
