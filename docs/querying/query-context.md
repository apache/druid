...
|allowedServerKeyword|`null`|Optionally get partial results from a subset of data servers whose hostname matches a given keyword. This is useful when you only want the queries to hit a subset of data servers for performance testing, debugging purposes, etc. E.g., this value can be set to "data-memory-tier" if that's a substring of the hostname convention of historicals you'd like to query and similarly can be set to "middle" to only gather results from peon tasks running in middle managers. Note that this feature can only further filter on data servers that are already been selected by the routing logic. In other words, it doesn't allow you to query arbitrary data servers ignoring `druid.broker.select.tier` and `druid.server.priority`.|
