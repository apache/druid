...
|useInMemoryBitmapInQuery|`false`|If both this flag and `enableInMemoryBitmap` is set to true in `tuningConfig` for realtime ingestion, queries can leverage bitmaps to avoid a full scan for applicable dimensions when the real time data is still in not persisted incremental index.|
