...
When performing math operations, Druid uses the integer datatype unless there are double or float values. If double or float values are involved, Druid uses double. Note that the highest precision way to store digits in Druid are 64-bit integers (long) or 64-bit floats (double). Specifically, a double can represent 52 binary digits or 15 decimal digits (2^52 is 4.5 x 10^15). 

For double values, Druid may return incorrect results if the number of digits 
