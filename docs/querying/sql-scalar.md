...
|`ARRAY_OF_DOUBLES_SKETCH_INTERSECT(expr, ..., [nominalEntries])`|Returns a union of tuple sketches, where each input expression must return a tuple sketch.  If the last value of the array is a numeric literal, Druid assumes that the value is an override parameter for [nominal entries](../development/extensions-core/datasketches-tuple.md).|
|`ARRAY_OF_DOUBLES_SKETCH_NOT(expr, ..., [nominalEntries])`|Returns an intersection of theta sketches, where each input expression must return a theta sketch.  If the last value of the array is a numeric literal, Druid assumes that the value is an override parameter for [nominal entries](../development/extensions-core/datasketches-tuple.md).|
|`ARRAY_OF_DOUBLES_SKETCH_UNION(expr, ..., [nominalEntries])`|Returns a set difference of theta sketches, where each input expression must return a theta sketch.  If the last value of the array is a numeric literal, Druid assumes that the value is an override parameter for [nominal entries](../development/extensions-core/datasketches-tuple.md).|
