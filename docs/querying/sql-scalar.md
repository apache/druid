...
|`LOOKUP(expr, lookupName[, replaceMissingValueWith])`|Look up `expr` in an already created [query-time lookup table](lookups.md) that has the name `lookupName`. If provided, the optional constant `replaceMissingValueWith` is returned when the `expr` is null or when the lookup does not contain a value for `expr`.<br /><br />You can query lookups directly using the [`lookup` schema](sql.md#from).|
