...
|`Truncated response context`|The response context is an internal data structure to store intermeidate state during query processing. Some of states are collected from query servers (such as historicals or realtime tasks) and merged in the Broker. To send those states to the Broker, the query servers serialize them in the HTTP header and send it together with the stream of intermediate query result. As the max size of an HTTP header is limited to 7 KB in Druid, this error can occur if JSON-serialized response context is larger than the limit.<br/><br/>This error shouldn't happen in most cases. A possible cause of this error can be too many segments moving (for segment balancing) at the time when a query is issued which reads lots of segments. You should be able to see debug logs about missing segments in the historical logs if you are facing this case.|
