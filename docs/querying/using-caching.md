...
As long as the service is set to populate the cache, you can set cache options for individual queries in the [query context reference](./query-context-reference.md). For example, you can `POST` a Druid SQL request to the HTTP POST API and include the context as a JSON object:
...
- [Query context reference](./query-context-reference.md) for more details and usage for the query context.
