...
|`mergeThreadLocal`|Whether merge buffers should always be split into thread-local buffers. Setting this to `true` uses memory reduces thread contention, but uses memory less efficiently.|false|
