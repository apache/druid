...
|`setProcessingThreadNames`|`false`| Whether processing thread names will be set to `processing_<queryId>` while processing a query. This aids in interpreting thread dumps, but has measurable overhead when segment scans are very quick. Set this to `true` to opt in to processing thread renaming. |
