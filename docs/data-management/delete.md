...
| `maxUsedStatusLastUpdatedTime` | null (no cutoff) | Represents the maximum timestamp used as a cutoff to include unused segments. The kill task considers segments in the specified `interval` marked as unused no later than this time. The default behavior is to kill all unused segments in the `interval` regardless of the time when segments where marked as unused.|
