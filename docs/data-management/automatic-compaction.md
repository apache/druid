...
To use compaction supervisors, update the [Coordinator compaction dynamic config]() and set:
  *  `useSupervisors` to `true` so that compaction tasks can be run as supervisor tasks
  *  `engine` to  `msq` to specify the MSQ task engine as the compaction engine or to `native` to use the native engine. This is the default engine if the `engine` field is omitted from the datasource compaction config.
