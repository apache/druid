...
\additionalauthors{Nishant Bangarwa (Metamarkets Group, Inc.,
{\texttt{nishant.bangarwa@metamarkets.com}}), Charles Allen, PhD (Metamarkets
Group, Inc., {\texttt{charles.allen@metamarkets.com}}), Eric Tschetter
...
in industry, and the long query times encountered when using batch frameworks
...
interactive user experience with data-driven applications queries must complete
in an order of milliseconds. Because most of these interactions revolve around
data exploration and computation, organizations quickly realized that in order
...
frequently selected as serving layers because of their general reliability in a
production setting. Solutions that involve these broad-focus technologies can
be inflexible once tailored to specific report cases, or suffer from
architecture drawbacks that prevent them from returning queries fast enough to
power interactive, user-facing applications \cite{tschetter2011druid}.

An ideal data serving layer alone will not suffice as a complete analytics
...
processed (transformed, or cleaned) before it is usable. The drawback of this
approach is that loading and processing batch data is slow, and insights on
events cannot be obtained until hours after the events have occurred
...
\begin{figure} 
\centering \includegraphics[width = 2.6in]{radstack}
\caption{
...
then merged together and the finalized segment is handed off.  Queries will hit
...
\caption{
Druid utilies multi-version concurrency control and reads data from segments
...
accurate results. Furthermore, even absent this restriction, Samza does not
...
Our batch processing pipeline is composed a multi-stage MapReduce pipeline. The
first set of jobs mirrors our streaming processing pipeline in that it
transforms data and joins relevant events in preparation for the is data to be
loaded into Druid. The second set of jobs is responsible for directly creating
immutable Druid segments. A final set of MapReduce\cite{dean2008mapreduce} jobs
that reads our processed data from the first set and indexes this data to
create Druid segments. The indexing code for both streaming and batch ingestion
in Druid is shared between the two modes of ingestion. These segments are then
uploaded to deep storage and registered with the  metadata store. Druid will
proceed to load the batch generated segments.
...
layer may choose to not include certain columns when it first processes a new
pipeline. If we want to include these columns in the future, we can reprocess
the raw data to generate new Druid segments. 
