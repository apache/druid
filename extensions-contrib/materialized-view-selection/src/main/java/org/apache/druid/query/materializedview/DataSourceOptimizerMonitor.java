...
public class DataSourceOptimizerMonitor extends AbstractMonitor 
{
  ...
  public boolean doMonitor(ServiceEmitter emitter) 
  {
    ...
    for (DataSourceOptimizerStats stat : stats) {
      ...
      emitter.emit(builder.setMetricAndValue("/materialized/view/query/totalNum", stat.getTotalcount()));
      emitter.emit(builder.setMetricAndValue("/materialized/view/query/hits", stat.getHitcount()));
      emitter.emit(builder.setMetricAndValue("/materialized/view/query/hitRate", stat.getHitRate()));
      emitter.emit(builder.setMetricAndValue("/materialized/view/select/avgCostMS", stat.getOptimizerCost()));
      ...
      for (Map.Entry<String, Long> derivative : derivativesStats.entrySet()) {
        ...
        emitter.emit(builder.setMetricAndValue("/materialized/view/derivative/numSelected", derivative.getValue()));
      }
      ...
      for (Set<String> fields : stat.getMissFields().keySet()) {
        ...
        emitter.emit(builder2.setMetricAndValue("/materialized/view/missNum", stat.getMissFields().get(fields).get()));
      }
    }
    return true;
  }
