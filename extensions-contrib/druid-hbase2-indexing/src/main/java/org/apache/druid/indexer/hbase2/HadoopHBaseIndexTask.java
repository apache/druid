...
public class HadoopHBaseIndexTask extends HadoopIndexTask
{
  ...
  private Map<String, String> getRemovingLibraries()
  {
    ...
    try {
      if (is == null) {
        Path path = Paths.get("./extensions/druid-hbase2-indexing/druid-hbase2-indexing.properties");
      ...
      removingLibMap = Arrays
          .stream(props.getProperty("hadoop.remove.lib.in.classpath", "hbase-1.").split(","))
          .map(lib -> {
