...
public class OssDataSegmentMover implements DataSegmentMover
{
  ...
  public DataSegment move(DataSegment segment, Map<String, Object> targetLoadSpec) throws SegmentLoadingException
  {
    try {
      ...
      final String targetS3Path = OssUtils.constructSegmentPath(
          targetKey,
      ...
      safeMove(bucket, key, targetBucket, targetS3Path);
  ...
  ) throws IOException, SegmentLoadingException
  {
    if (srcBucket.equals(dstBucket) && srcPath.equals(dstPath)) {
      log.info("No need to move file[s3://%s/%s] onto itself", srcBucket, srcPath);
      return;
    }
    if (client.doesObjectExist(srcBucket, srcPath)) {
      ...
      if (listResult.getObjectSummaries().size() == 0) {
        // should never happen
        throw new ISE("Unable to list object [s3://%s/%s]", srcBucket, srcPath);
      }
      ...
      if (objectSummary.getStorageClass() != null &&
        ...
        throw new OSSException(
            StringUtils.format(
                "Cannot move file[oss://%s/%s] of storage class glacier, skipping.",
                srcBucket,
                srcPath
            )
        );
