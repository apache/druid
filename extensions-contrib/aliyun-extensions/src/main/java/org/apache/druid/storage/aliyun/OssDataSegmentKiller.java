...
public class OssDataSegmentKiller implements DataSegmentKiller
{
  ...
  public void kill(DataSegment segment) throws SegmentLoadingException
  {
    try {
      ...
      if (client.doesObjectExist(s3Bucket, s3Path)) {
        log.info("Removing index file[s3://%s/%s] from s3!", s3Bucket, s3Path);
        client.deleteObject(s3Bucket, s3Path);
      }
      ...
      if (client.doesObjectExist(s3Bucket, s3DescriptorPath)) {
        log.info("Removing descriptor file[s3://%s/%s] from s3!", s3Bucket, s3DescriptorPath);
        client.deleteObject(s3Bucket, s3DescriptorPath);
      }
    }
