...
public class OssTimestampVersionedDataFinderTest
{
  ...
  public void testSimpleLatestVersion()
  {
    ...
    URI latest = finder.getLatestVersion(URI.create(StringUtils.format("oss://%s/%s", bucket, keyPrefix)), pattern);

    EasyMock.verify(client);

    URI expected = URI.create(StringUtils.format("oss://%s/%s", bucket, object1.getKey()));
  ...
  public void testFindSelf()
  {
    ...
    URI expected = URI.create(StringUtils.format("oss://%s/%s", bucket, object0.getKey()));
  ...
  public void testFindExact()
  {
    ...
    URI latest = finder.getLatestVersion(URI.create(StringUtils.format("oss://%s/%s", bucket, object0.getKey())), null);
