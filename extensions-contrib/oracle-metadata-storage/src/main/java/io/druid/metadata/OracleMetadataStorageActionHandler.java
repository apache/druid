...
public class OracleMetadataStorageActionHandler<EntryType, StatusType, LogType, LockType>
    implements MetadataStorageActionHandler<EntryType, StatusType, LogType, LockType>
{
  private static final EmittingLogger log = new EmittingLogger(OracleMetadataStorageActionHandler.class);
  ...
  public OracleMetadataStorageActionHandler(
  ...
  private boolean addLock(Handle handle, String entryId, LockType lock) throws JsonProcessingException
  {
    final String statement = StringUtils.format(
        "INSERT INTO %1$s (id, %2$s_id, lock_payload) VALUES (%1$s_seq.nextval, :entryId, :payload)",
    ...
    return connector.retryWithHandle(
        new HandleCallback<Boolean>()
        {
          ...
          public Boolean withHandle(Handle handle) throws Exception
          {
            return handle.createStatement(
                StringUtils.format(
                    "INSERT INTO %1$s (id, %2$s_id, log_payload) VALUES (%1$s_seq.nextval, :entryId, :payload)",
