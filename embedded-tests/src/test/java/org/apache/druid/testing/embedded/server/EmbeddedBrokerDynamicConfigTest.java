...
/**
 * Integration test for broker dynamic configuration, covering {@code queryBlocklist}
...
public class EmbeddedBrokerDynamicConfigTest extends EmbeddedClusterTestBase
{
  ...
  public void testBlacklistedDataNodesExcludesNodesFromQueryRouting()
  {
    ...
    Assertions.assertTrue(initialCount > 0, "Expected non-zero row count before blacklisting");

    // Blacklist the historical node
    String historicalHostAndPort = historical.bindings().selfNode().getHostAndPort();
    updateBrokerDynamicConfig(
        BrokerDynamicConfig.builder()
    ...
    String blacklistedResult = cluster.callApi().runSql("SELECT COUNT(*) FROM %s", dataSource);
    int blacklistedCount = Integer.parseInt(blacklistedResult.trim());
    Assertions.assertEquals(0, blacklistedCount, "Expected 0 rows when historical is blacklisted");

    // Clear the blacklist and verify data is accessible again
    updateBrokerDynamicConfig(BrokerDynamicConfig.builder().build());
    String restoredResult = cluster.callApi().runSql("SELECT COUNT(*) FROM %s", dataSource);
    int restoredCount = Integer.parseInt(restoredResult.trim());
    Assertions.assertEquals(initialCount, restoredCount, "Expected row count restored after clearing blacklist");
  }
