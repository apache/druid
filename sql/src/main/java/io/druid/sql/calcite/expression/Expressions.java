  ...
  )
  {
    ...
    if (kind == SqlKind.LIKE) {
      ...
      if (column.equals(Column.TIME_COLUMN_NAME) && ExtractionFns.toQueryGranularity(extractionFn) != null) {
        ...
        final Granularity gran = ExtractionFns.toQueryGranularity(extractionFn);
        final long rhsMillis = ((Calendar) RexLiteral.value(rhs)).getTimeInMillis();
        final long truncatedRhsMillis = gran.truncate(new DateTime(rhsMillis)).getMillis();
        if (truncatedRhsMillis != rhsMillis) {
          // Nothing matches.
          return Filtration.matchNothing();
        } else {
          ...
          return new BoundDimFilter(
              ...
              String.valueOf(truncatedRhsMillis),
              String.valueOf(gran.increment(gran.truncate(new DateTime(rhsMillis))).getMillis()),
              false,
              true,
              null,
              null,
