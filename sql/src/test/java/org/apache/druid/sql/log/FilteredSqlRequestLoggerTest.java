...
public class FilteredSqlRequestLoggerTest
{
  @Test
  public void testFilterBelowThreshold() throws IOException
  {
    SqlRequestLogger delegate = EasyMock.createStrictMock(SqlRequestLogger.class);
    delegate.log(EasyMock.anyObject());
    EasyMock.expectLastCall().andThrow(new IOException());
    FilteredSqlRequestLoggerProvider.FilteredSqlRequestLogger logger = new FilteredSqlRequestLoggerProvider.FilteredSqlRequestLogger(
        delegate,
        1000
    );
    SqlRequestLogLine requestLogLine = EasyMock.createMock(SqlRequestLogLine.class);
    EasyMock.expect(requestLogLine.getQueryStats())
            .andReturn(new QueryStats(ImmutableMap.of("sqlQuery/time", 100)))
            .once();
