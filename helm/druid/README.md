...
```bash
helm -n druid status "druid"
```

If you want to delete your Chart, use this command:

```bash
helm -n druid delete --purge "druid"
```
...
| `image.tag`                              | container image tag                                          | `0.21.1`                    |
| `image.pullPolicy`                       | container pull policy                                        | `IfNotPresent`              |
| `image.pullSecrets`                      | image pull secrest for private repositoty                    | `[]`                        |
| `configMap.enabled`                      | enable druid global configuration as configmap               | `true`                      |
| `globalConfig`                           | druid configuration variables for all components             | ``                          |
...
| `hdfs.enabled`                           | enable druid hdfs configuration to be mount in historical node and middle-manager node (Please add related extension) | `false`                     |
| `hdfs.configMapName`                     | configmap's name related to hdfs that contains core-site.xml and hdfs-site.xml configuration | ``                          |
