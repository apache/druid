    ...
    for (String columnName : columnNames) {
      ...
      switch (type) {
        ...
        default:
          ...
          analysis = ColumnAnalysis.error(StringUtils.safeFormat("unknown_type_%s", type));
      }
  ...
  )
  {
    try (final ComplexColumn complexColumn = column != null ? column.getComplexColumn() : null) {
      ...
      if (analyzingSize() && complexColumn != null) {
        ...
        if (serde == null) {
          return ColumnAnalysis.error(StringUtils.safeFormat("unknown_complex_%s", typeName));
        }
