...
public final class DimensionHandlerUtil
{
  ...
  public static DimensionHandler getHandlerFromCapabilities(String dimensionName, ColumnCapabilities capabilities)
  {
    ...
    if (capabilities.getType() == ValueType.LONG) {
      if (capabilities.isDictionaryEncoded() || capabilities.hasBitmapIndexes()) {
        throw new IAE("Long column does not support dictionary encoding or bitmap index.");
      }
      handler = new LongDimensionHandler(dimensionName);
    }
    if (capabilities.getType() == ValueType.FLOAT) {
      if (capabilities.isDictionaryEncoded() || capabilities.hasBitmapIndexes()) {
        throw new IAE("Float column does not support dictionary encoding or bitmap index.");
      }
  ...
  public static Function getTransformerForType(ValueType type)
  {
    ...
    switch (type) {
      case LONG:
        valueTransformer = LongDimensionHandler.LONG_TRANSFORMER;
        break;
      case FLOAT:
        valueTransformer = FloatDimensionHandler.FLOAT_TRANSFORMER;
        break;
      case STRING:
