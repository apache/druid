...
public abstract class IncrementalIndex<AggregatorType> implements Iterable<Row>, Closeable
{
  ...
  TimeAndDims toTimeAndDims(InputRow row) throws IndexSizeExceededException
  {
    ...
    synchronized (dimensionDescs) {
      ...
      for (String dimension : rowDimensions) {
        ...
        Object rawDimVal = row.getRaw(dimension);
        Object dimsKey = (rawDimVal instanceof List) ?
                         indexer.processRowValsListToIndexKey(rawDimVal) :
                         indexer.processSingleRowValToIndexKey(rawDimVal);

        // Set column capabilities as data is coming in
        if (!capabilities.hasMultipleValues() && dimsKey != null && handler.getLengthFromArrayObject(dimsKey) > 1) {
  ...
  public void specifyDimensionCapability(String dimension, ValueType type)
  {
    ColumnCapabilitiesImpl capabilities = makeCapabilitesFromValueType(type);
    columnCapabilities.put(dimension, capabilities);
  }
            ...
            new Function<Map.Entry<TimeAndDims, Integer>, Row>()
            {
              ...
              public Row apply(final Map.Entry<TimeAndDims, Integer> input)
              {
                ...
                for (int i = 0; i < theDims.length; ++i) {
                  ...
                  if (dim == null || handler.getLengthFromArrayObject(dim) == 0) {
                    theVals.put(dimensionName, null);
                    continue;
                  }
                  final DimensionIndexer indexer = dimensionDesc.getIndexer();
                  Object rowVals = indexer.convertTimeAndDimsKeyToActualArray(dim);
  ...
  public static final class TimeAndDims
  {
    ...
    public boolean equals(Object o)
    {
      ...
      for (int i = 0; i < dims.length; i++) {
        ...
        if (!indexer.checkTimeAndDimsKeyEqual(dims[i], that.dims[i])) {
          return false;
        }
      }
      return true;
    ...
    public int hashCode()
    {
      ...
      for (int i = 0; i < dims.length; i++) {
        final DimensionIndexer indexer = dimensionDescsList.get(i).getIndexer();
        hash = 31 * hash + indexer.getTimeAndDimsKeyHashcode(dims[i]);
      }
      return hash;
    }
  }
  ...
  static final class TimeAndDimsComp implements Comparator<TimeAndDims>
  {
    ...
    public int compare(TimeAndDims lhs, TimeAndDims rhs)
    {
      ...
      while (retVal == 0 && index < numComparisons) {
        ...
        retVal = indexer.compareTimeAndDimsKey(lhsIdxs, rhsIdxs);
        ++index;
      }
