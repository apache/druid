    ...
    for (IncrementalIndex.TimeAndDims timeAndDims : index.getFacts().keySet()) {
      ...
      for (IncrementalIndex.DimensionDesc dimension : dimensions) {
        ...
        if (dimIndex >= dims.length || dims[dimIndex] == null) {
          accessor.indexer.processSingleRowValToIndexKey(null);
          continue;
        }
        ...
        if(capabilities.hasBitmapIndexes()) {
          ...
          indexer.fillBitmapsFromTimeAndDimsKey(dims[dimIndex], rowNum, bitmapIndexes, bitmapFactory);
        }
      }
      ++rowNum;
    }
            ...
            new Function<Map.Entry<IncrementalIndex.TimeAndDims, Integer>, Rowboat>()
            {
              ...
              public Rowboat apply(Map.Entry<IncrementalIndex.TimeAndDims, Integer> input)
              {
                ...
                for (IncrementalIndex.DimensionDesc dimension : dimensions) {
                  ...
                  Object sortedDimVals = indexer.convertTimeAndDimsKeyToSortedEncodedArray(dimValues[dimIndex]);
