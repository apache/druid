...
/**
 */
public class CompressedIntsSupplierSerializer
{
  public static CompressedIntsSupplierSerializer create(
  ...
  {
    final CompressedIntsSupplierSerializer retVal = new CompressedIntsSupplierSerializer(
        CompressedIntsIndexedSupplier.MAX_INTS_IN_BUFFER,
        new GenericIndexedWriter<ResourceHolder<IntBuffer>>(
            ioPeon, filenameBase, CompressedIntBufferObjectStrategy.getBufferForOrder(order, compression, CompressedIntsIndexedSupplier.MAX_INTS_IN_BUFFER)
        ),
        compression
    );
    return retVal;
  ...
  public CompressedIntsSupplierSerializer(
      int sizePer,
  ...
  )
  {
    ...
    endBuffer = IntBuffer.allocate(sizePer);
    endBuffer.mark();
  }
  ...
  public void closeAndConsolidate(OutputSupplier<? extends OutputStream> consolidatedOut) throws IOException
  {
    ...
    try (OutputStream out = consolidatedOut.getOutput()) {
      out.write(CompressedIntsIndexedSupplier.version);
