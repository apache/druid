...
public class Filters
{
  ...
  private static Filter flatten(Filter root) {
    if (root instanceof Filter.RelationalFilter) {
      List<Filter> children = ((Filter.RelationalFilter)root).getChildren();
      ...
      for (int i = 0; i < children.size(); ++i) {
        ...
        if (child.getClass() == root.getClass() && !(child instanceof NotFilter)) {
          boolean first = true;
          List<Filter> grandKids = ((Filter.RelationalFilter)child).getChildren();
