...
import org.apache.druid.segment.nested.StandardNestedDataColumn;
...
public class StandardNestedColumnSupplier implements Supplier<StandardTypeColumn>
{
  public static StandardNestedColumnSupplier read(
      String columnName,
      boolean hasNulls,
      ByteBuffer bb,
  ...
  )
  {
    ...
    if (version == StandardTypeColumnSerializer.V0) {
      try {
        ...
        final ByteBuffer rawBuffer = StandardTypeColumnPartSerde.loadInternalFile(
            mapper,
            columnName,
            StandardTypeColumnSerializer.RAW_FILE_NAME
        );
        compressedRawColumnSupplier = CompressedVariableSizedBlobColumnSupplier.fromByteBuffer(
            StandardTypeColumnSerializer.getInternalFileName(
                columnName,
                StandardTypeColumnSerializer.RAW_FILE_NAME
            ),
            rawBuffer,
        ...
        if (hasNulls) {
          ...
          final ByteBuffer nullIndexBuffer = StandardTypeColumnPartSerde.loadInternalFile(
              mapper,
              columnName,
              StandardTypeColumnSerializer.NULL_BITMAP_FILE_NAME
          );
        ...
        return new StandardNestedColumnSupplier(
            columnName,
            fields,
            fieldInfo,
  ...
  private StandardNestedColumnSupplier(
      String columnName,
  ...
  public StandardTypeColumn get()
  {
    if (frontCodedStringDictionarySupplier != null) {
      return new StandardNestedDataColumn<>(
          columnName,
          getLogicalType(),
          columnConfig,
          compressedRawColumnSupplier,
