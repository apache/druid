...
public class StandardLongColumnSerializer extends StandardTypeColumnSerializer
{
  private static final Logger log = new Logger(StandardLongColumnSerializer.class);
  ...
  public StandardLongColumnSerializer(
      String name,
      IndexSpec indexSpec,
  ...
  private void writeBitmapIndexes() throws IOException
  {
    if (!closedForWrite) {
      for (int i = 0; i < bitmaps.length; i++) {
