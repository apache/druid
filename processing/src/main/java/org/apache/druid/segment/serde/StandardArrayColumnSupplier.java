...
public class StandardArrayColumnSupplier implements Supplier<StandardTypeColumn>, ColumnIndexSupplier
{
  public static StandardArrayColumnSupplier read(
      String columnName,
  ...
  )
  {
    ...
    if (version == StandardTypeColumnSerializer.V0) {
      try {
        ...
        final ByteBuffer stringDictionaryBuffer = StandardTypeColumnPartSerde.loadInternalFile(
            mapper,
            columnName,
            StandardTypeColumnSerializer.STRING_DICTIONARY_FILE_NAME
        );
        ...
        final ByteBuffer encodedValueColumn = StandardTypeColumnPartSerde.loadInternalFile(
            mapper,
            columnName,
            StandardTypeColumnSerializer.ENCODED_VALUE_COLUMN_FILE_NAME
        );
        final CompressedVSizeColumnarIntsSupplier ints = CompressedVSizeColumnarIntsSupplier.fromByteBuffer(
            encodedValueColumn,
            byteOrder
        );
        final ByteBuffer longDictionaryBuffer = StandardTypeColumnPartSerde.loadInternalFile(
            mapper,
            columnName,
            StandardTypeColumnSerializer.LONG_DICTIONARY_FILE_NAME
        );
        final ByteBuffer doubleDictionaryBuffer = StandardTypeColumnPartSerde.loadInternalFile(
            mapper,
            columnName,
        ...
        final ByteBuffer valueIndexBuffer = StandardTypeColumnPartSerde.loadInternalFile(
            mapper,
            columnName,
            StandardTypeColumnSerializer.BITMAP_INDEX_FILE_NAME
        );
        ...
        final ByteBuffer arrayDictionarybuffer = StandardTypeColumnPartSerde.loadInternalFile(
            mapper,
            columnName,
            StandardTypeColumnSerializer.ARRAY_DICTIONARY_FILE_NAME
        );
        arrayDictionarySupplier = FrontCodedIntArrayIndexed.read(
            arrayDictionarybuffer,
            byteOrder
        );
        return new StandardArrayColumnSupplier(
            logicalType,
            stringDictionary,
  ...
  public StandardArrayColumnSupplier(
      ColumnType logicalType,
  ...
  @Override
  public StandardTypeColumn get()
  {
    if (frontCodedStringDictionarySupplier != null) {
      return new StandardArrayColumn<>(
