...
public class RowBasedExpressionColumnValueSelector extends ExpressionColumnValueSelector
{
  ...
  private final Expr.ExprInputBindingAnalysis baseExprInputBindingAnalysis;
  ...
  public RowBasedExpressionColumnValueSelector(
      Expr expression,
      Expr.ExprInputBindingAnalysis baseExprInputBindingAnalysis,
  ...
  )
  {
    ...
    this.unknownColumns = unknownColumnsSet.stream()
                                           .filter(x -> !baseExprInputBindingAnalysis.getArrayBindings().contains(x))
                                           .collect(Collectors.toList());
    this.baseExprInputBindingAnalysis = baseExprInputBindingAnalysis;
  ...
  public ExprEval getObject()
  {
    ...
    if (arrayBindings.size() > 0) {
      ...
      Expr transformed = Parser.applyUnappliedBindings(expression, baseExprInputBindingAnalysis, arrayBindings);
