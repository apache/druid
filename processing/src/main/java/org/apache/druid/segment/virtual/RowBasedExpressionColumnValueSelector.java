...
public class RowBasedExpressionColumnValueSelector extends ExpressionColumnValueSelector
{
  ...
  )
  {
    ...
    this.unknownColumns = unknownColumnsSet.stream()
                                           .filter(x -> !baseExprExprInputBindingAnalysis.getArrayBindings().contains(x))
                                           .collect(Collectors.toList());
    this.baseExprExprInputBindingAnalysis = baseExprExprInputBindingAnalysis;
  ...
  public ExprEval getObject()
  {
    ...
    if (arrayBindings.size() > 0) {
      ...
      Expr transformed = Parser.applyUnappliedBindings(expression, baseExprExprInputBindingAnalysis, arrayBindings);
