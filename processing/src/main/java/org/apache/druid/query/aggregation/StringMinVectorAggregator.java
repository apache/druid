...
public class StringMinVectorAggregator implements VectorAggregator
{
  ...
  public StringMinVectorAggregator(
    ...
    for (int i = startRow; i < endRow; i++) {
      if (multiValueSelector != null) {
        ...
        if (i < rows.length) {
          ...
          updateMinValue(buf, position, foundValue);
        }
  ...
  private void updateMinValue(ByteBuffer buf, int position, @Nullable String foundValue)
  {
    int currentFlag = buf.getInt(position);
    if (currentFlag == NOT_FOUND_FLAG_VALUE) {
      setMinStringInBuffer(buf, position, foundValue);
      return;
    }

    String currentMinValue = getMinStringInBuffer(buf, position);
    if (foundValue != null && (currentMinValue == null || foundValue.compareTo(currentMinValue) < 0)) {
      setMinStringInBuffer(buf, position, foundValue);
    }
  }
  ...
  private void setMinStringInBuffer(ByteBuffer buf, int position, @Nullable String value)
  {
    if (value == null) {
