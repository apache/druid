...
/**
 * SIMD specialization of {@link LongMinVectorAggregator}'s ungrouped contiguous-range aggregation. The hot loop
 * issues a hardcoded {@link LongVector#min} and a {@code reduceLanes(VectorOperators.MIN)} so the JIT emits the
 * platform's long-min and long-min-reduce intrinsics. Null lanes preserve the lane's seeded {@link Long#MAX_VALUE}
 * via masked {@code lanewise} so the reduction is unaffected by them.
 */
public final class SimdLongMinVectorAggregator extends LongMinVectorAggregator implements NullAwareVectorAggregator
{
  ...
  public boolean aggregate(ByteBuffer buf, int position, int startRow, int endRow, boolean[] nullVector)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      ...
      vacc = vacc.lanewise(VectorOperators.MIN, LongVector.fromArray(SPECIES, vector, i), notNull);
