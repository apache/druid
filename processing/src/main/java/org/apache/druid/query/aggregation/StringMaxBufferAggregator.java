...
public class StringMaxBufferAggregator implements BufferAggregator
{
  private final BaseObjectColumnValueSelector<String> selector;
  private final int maxStringBytes;

  public StringMaxBufferAggregator(BaseObjectColumnValueSelector<String> selector, int maxStringBytes)
  {
  ...
  public void aggregate(ByteBuffer buf, int position)
  {
    ...
    if (buf.getInt(position) == NOT_FOUND_FLAG_VALUE) {
      if (currentValue == null) {
        ...
        setMaxStringInBuffer(buf, position, currentValue);
      }
      return;
    }

    // Retrieve the current maximum from the buffer
    String currentMaxValue = getMaxStringInBuffer(buf, position);

    // Update the buffer with the new maximum if the current value is larger
    if (currentValue != null && (currentMaxValue == null || currentValue.compareTo(currentMaxValue) > 0)) {
      setMaxStringInBuffer(buf, position, currentValue);
    }
  }
  ...
  {
    throw new UnsupportedOperationException("StringMaxBufferAggregator does not support getFloat()");
  }

  @Override
  public long getLong(ByteBuffer buf, int position)
  {
    throw new UnsupportedOperationException("StringMaxBufferAggregator does not support getLong()");
  }

  @Override
  public double getDouble(ByteBuffer buf, int position)
  {
    throw new UnsupportedOperationException("StringMaxBufferAggregator does not support getDouble()");
  }
  ...
  private void setMaxStringInBuffer(ByteBuffer buf, int position, String value)
  {
