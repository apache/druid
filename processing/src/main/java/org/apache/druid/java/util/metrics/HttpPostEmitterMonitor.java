...
public class HttpPostEmitterMonitor extends FeedDefiningMonitor
{
  ...
  public boolean doMonitor(ServiceEmitter emitter)
  {
    ...
    emitter.emit(builder.setMetricAndValue("emitter/events/emitQueue", httpPostEmitter.getEventsToEmit()));
    emitter.emit(builder.setMetricAndValue("emitter/events/large/emitQueue", httpPostEmitter.getLargeEventsToEmit()));

    emitter.emit(builder.setMetricAndValue("emitter/buffers/emitQueue", httpPostEmitter.getBuffersToEmit()));

    emitter.emit(builder.setMetricAndValue("emitter/buffers/reuseQueue", httpPostEmitter.getBuffersToReuse()));
  ...
  private void emitEmittedEvents(ServiceEmitter emitter)
  {
    ...
    emitter.emit(builder.setMetricAndValue("emitter/events/emitted/delta", emittedEventsDelta));
  ...
  private void emitDroppedBuffers(ServiceEmitter emitter)
  {
    ...
    emitter.emit(builder.setMetricAndValue("emitter/buffers/dropped/delta", droppedBuffersDelta));
  ...
  private void emitAllocatedBuffers(ServiceEmitter emitter)
  {
    ...
    emitter.emit(builder.setMetricAndValue("emitter/buffers/allocated/delta", allocatedBuffersDelta));
  ...
  private void emitFailedBuffers(ServiceEmitter emitter)
  {
    ...
    emitter.emit(builder.setMetricAndValue("emitter/buffers/failed/delta", failedBuffersDelta));
  ...
  private void emitTimeCounterMetrics(ServiceEmitter emitter, ConcurrentTimeCounter timeCounter, String metricNameBase)
  {
    ...
    if (count != 0) {
      emitter.emit(builder.setMetricAndValue(metricNameBase + "timeMsSum", timeSum));
      emitter.emit(builder.setMetricAndValue(metricNameBase + "count", count));
    }
    Integer maxTime = timeCounter.getAndResetMaxTime();
    if (maxTime != null) {
      emitter.emit(builder.setMetricAndValue(metricNameBase + "maxTimeMs", maxTime));
    }
    Integer minTime = timeCounter.getAndResetMinTime();
    if (minTime != null) {
      emitter.emit(builder.setMetricAndValue(metricNameBase + "minTimeMs", minTime));
    }
  }
