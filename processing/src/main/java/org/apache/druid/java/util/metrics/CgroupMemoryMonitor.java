...
public class CgroupMemoryMonitor extends FeedDefiningMonitor
{
  ...
  public boolean doMonitor(ServiceEmitter emitter)
  {
    ...
    stat.getMemoryStats().forEach((key, value) -> {
      ...
      emitter.emit(builder.setMetricAndValue(StringUtils.format("cgroup/memory/%s", key), value));
    });
    stat.getNumaMemoryStats().forEach((key, value) -> {
      ...
      MonitorUtils.addDimensionsToBuilder(builder, dimensions);
      value.forEach((k, v) -> emitter.emit(builder.setMetricAndValue(StringUtils.format("cgroup/memory_numa/%s/pages", k), v)));
    });
    return true;
  }
}
