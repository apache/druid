...
public class JvmMonitor extends FeedDefiningMonitor
{
  ...
  private void emitThreadAllocationMetrics(ServiceEmitter emitter)
  {
    ...
    if (collector != null) {
      ...
      emitter.emit(builder.setMetricAndValue("jvm/heapAlloc/bytes", delta));
    }
  }
  ...
  private void emitJvmMemMetrics(ServiceEmitter emitter)
  {
    ...
    for (Map.Entry<String, MemoryUsage> entry : usages.entrySet()) {
      ...
      emitter.emit(builder.setMetricAndValue("jvm/mem/max", usage.getMax()));
      emitter.emit(builder.setMetricAndValue("jvm/mem/committed", usage.getCommitted()));
      emitter.emit(builder.setMetricAndValue("jvm/mem/used", usage.getUsed()));
      emitter.emit(builder.setMetricAndValue("jvm/mem/init", usage.getInit()));
    }
    ...
    for (MemoryPoolMXBean pool : ManagementFactory.getMemoryPoolMXBeans()) {
      ...
      emitter.emit(builder.setMetricAndValue("jvm/pool/max", usage.getMax()));
      emitter.emit(builder.setMetricAndValue("jvm/pool/committed", usage.getCommitted()));
      emitter.emit(builder.setMetricAndValue("jvm/pool/used", usage.getUsed()));
      emitter.emit(builder.setMetricAndValue("jvm/pool/init", usage.getInit()));
    }
  }
  ...
  private void emitDirectMemMetrics(ServiceEmitter emitter)
  {
    for (BufferPoolMXBean pool : ManagementFactory.getPlatformMXBeans(BufferPoolMXBean.class)) {
      ...
      emitter.emit(builder.setMetricAndValue("jvm/bufferpool/capacity", pool.getTotalCapacity()));
      emitter.emit(builder.setMetricAndValue("jvm/bufferpool/used", pool.getMemoryUsed()));
      emitter.emit(builder.setMetricAndValue("jvm/bufferpool/count", pool.getCount()));
    }
  }
  ...
  private class GcGenerationCollector
  {
    ...
    void emit(ServiceEmitter emitter, Map<String, String[]> dimensions)
    {
      ...
      emitter.emit(builder.setMetricAndValue("jvm/gc/count", newInvocations - lastInvocations));
      ...
      emitter.emit(builder.setMetricAndValue("jvm/gc/cpu", (newCpuMillis - lastCpuMillis) * 1_000_000L));
  ...
  private class GcGenerationSpace
  {
    ...
    void emit(ServiceEmitter emitter, Map<String, String[]> dimensions)
    {
      ...
      emitter.emit(builder.setMetricAndValue("jvm/gc/mem/max", memoryUsage.getMax()));
      emitter.emit(builder.setMetricAndValue("jvm/gc/mem/capacity", memoryUsage.getCommitted()));
      emitter.emit(builder.setMetricAndValue("jvm/gc/mem/used", memoryUsage.getUsed()));
      emitter.emit(builder.setMetricAndValue("jvm/gc/mem/init", memoryUsage.getInit()));
    }
  }
}
