...
{
  public SimdDoubleAcosProcessor(ExprVectorProcessor<?> input, DoubleUnivariateDoubleFunction scalarFallback)
  {
  ...
  protected void processVector(double[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    if (i < currentSize) {
      ...
      DoubleVector.fromArray(SPECIES, input, i, mask).lanewise(VectorOperators.ACOS).intoArray(outValues, i, mask);
    }
    if (inputNulls == null) {
