...
/**
 * SIMD specialization of {@code (double[]) -> double[]} square root. {@code lanewise(VectorOperators.SQRT)} is
 * emitted with the operator literally inline so the JIT statically resolves it to the platform's double-sqrt
 * intrinsic (e.g. {@code vsqrtpd} on x86).
 */
public final class SimdDoubleSqrtProcessor extends SimdDoubleUnaryProcessor
{
  public SimdDoubleSqrtProcessor(ExprVectorProcessor<?> input, DoubleUnivariateDoubleFunction scalarFallback)
  {
  ...
  protected void processVector(double[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      DoubleVector.fromArray(SPECIES, input, i).lanewise(VectorOperators.SQRT).intoArray(outValues, i);
    }
    for (; i < currentSize; i++) {
