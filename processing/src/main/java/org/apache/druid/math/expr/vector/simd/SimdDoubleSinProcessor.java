...
/**
 * SIMD specialization of {@code (double[]) -> double[]} sine. See {@link SimdSupportedUnaryOp#SIN} for the
 * shared VO_MATHLIB performance notes.
 */
public final class SimdDoubleSinProcessor extends SimdDoubleUnaryProcessor
{
  public SimdDoubleSinProcessor(ExprVectorProcessor<?> input, DoubleUnivariateDoubleFunction scalarFallback)
  {
  ...
  protected void processVector(double[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    if (i < currentSize) {
      ...
      DoubleVector.fromArray(SPECIES, input, i, mask).lanewise(VectorOperators.SIN).intoArray(outValues, i, mask);
    }
    if (inputNulls == null) {
