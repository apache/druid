...
/**
 * SIMD specialization of {@code (long[]) -> double[]} {@code exp(x)-1}. See {@link SimdSupportedUnaryOp#EXPM1}
 * for the shared VO_MATHLIB performance notes.
 */
public final class SimdLongToDoubleExpm1Processor extends SimdLongToDoubleUnaryProcessor
{
  public SimdLongToDoubleExpm1Processor(ExprVectorProcessor<?> input, DoubleUnivariateLongFunction scalarFallback)
  {
  ...
  protected void processVector(long[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      ...
      va.lanewise(VectorOperators.EXPM1).intoArray(outValues, i);
    }
    if (i < currentSize) {
      ...
      va.lanewise(VectorOperators.EXPM1).intoArray(outValues, i, doubleMask);
    }
    if (inputNulls == null) {
