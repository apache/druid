...
/**
 * SIMD specialization of {@code (double[]) -> double[]} {@code log(1+x)}. See {@link SimdSupportedUnaryOp#LOG1P}
 * for the shared VO_MATHLIB performance notes.
 */
public final class SimdDoubleLog1pProcessor extends SimdDoubleUnaryProcessor
{
  public SimdDoubleLog1pProcessor(ExprVectorProcessor<?> input, DoubleUnivariateDoubleFunction scalarFallback)
  {
  ...
  protected void processVector(double[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      DoubleVector.fromArray(SPECIES, input, i).lanewise(VectorOperators.LOG1P).intoArray(outValues, i);
    }
    if (i < currentSize) {
      final VectorMask<Double> mask = SPECIES.indexInRange(i, currentSize);
      DoubleVector.fromArray(SPECIES, input, i, mask).lanewise(VectorOperators.LOG1P).intoArray(outValues, i, mask);
    }
    if (inputNulls == null) {
