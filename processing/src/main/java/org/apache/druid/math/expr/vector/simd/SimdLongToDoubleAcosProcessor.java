...
/**
 * SIMD specialization of {@code (long[]) -> double[]} arc cosine. See {@link SimdSupportedUnaryOp#ACOS} for the
 * shared VO_MATHLIB performance notes.
 */
public final class SimdLongToDoubleAcosProcessor extends SimdLongToDoubleUnaryProcessor
{
  public SimdLongToDoubleAcosProcessor(ExprVectorProcessor<?> input, DoubleUnivariateLongFunction scalarFallback)
  {
  ...
  protected void processVector(long[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      ...
      va.lanewise(VectorOperators.ACOS).intoArray(outValues, i);
    }
    if (i < currentSize) {
      ...
      va.lanewise(VectorOperators.ACOS).intoArray(outValues, i, doubleMask);
    }
    if (inputNulls == null) {
