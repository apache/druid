...
/**
 * SIMD specialization of {@code (double[]) -> double[]} cosine. See {@link SimdSupportedUnaryOp#COS} for the
 * shared VO_MATHLIB performance notes.
 */
public final class SimdDoubleCosProcessor extends SimdDoubleUnaryProcessor
{
  public SimdDoubleCosProcessor(ExprVectorProcessor<?> input, DoubleUnivariateDoubleFunction scalarFallback)
  {
  ...
  protected void processVector(double[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      DoubleVector.fromArray(SPECIES, input, i).lanewise(VectorOperators.COS).intoArray(outValues, i);
    }
    if (i < currentSize) {
      final VectorMask<Double> mask = SPECIES.indexInRange(i, currentSize);
      DoubleVector.fromArray(SPECIES, input, i, mask).lanewise(VectorOperators.COS).intoArray(outValues, i, mask);
    }
    if (inputNulls == null) {
