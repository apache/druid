...
/**
 * SIMD specialization of {@code (long[]) -> double[]} {@code log(1+x)}. See {@link SimdSupportedUnaryOp#LOG1P}
 * for the shared VO_MATHLIB performance notes.
 */
public final class SimdLongToDoubleLog1pProcessor extends SimdLongToDoubleUnaryProcessor
{
  public SimdLongToDoubleLog1pProcessor(ExprVectorProcessor<?> input, DoubleUnivariateLongFunction scalarFallback)
  {
  ...
  protected void processVector(long[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      ...
      va.lanewise(VectorOperators.LOG1P).intoArray(outValues, i);
    }
    if (i < currentSize) {
      ...
      va.lanewise(VectorOperators.LOG1P).intoArray(outValues, i, doubleMask);
    }
    if (inputNulls == null) {
