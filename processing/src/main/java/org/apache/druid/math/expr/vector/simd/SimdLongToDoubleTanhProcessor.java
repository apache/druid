...
/**
 * SIMD specialization of {@code (long[]) -> double[]} hyperbolic tangent. See {@link SimdSupportedUnaryOp#TANH}
 * for the shared VO_MATHLIB performance notes.
 */
public final class SimdLongToDoubleTanhProcessor extends SimdLongToDoubleUnaryProcessor
{
  public SimdLongToDoubleTanhProcessor(ExprVectorProcessor<?> input, DoubleUnivariateLongFunction scalarFallback)
  {
  ...
  protected void processVector(long[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      ...
      va.lanewise(VectorOperators.TANH).intoArray(outValues, i);
    }
    if (i < currentSize) {
      ...
      va.lanewise(VectorOperators.TANH).intoArray(outValues, i, doubleMask);
    }
    if (inputNulls == null) {
