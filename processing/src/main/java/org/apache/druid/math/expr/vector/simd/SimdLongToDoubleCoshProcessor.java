...
/**
 * SIMD specialization of {@code (long[]) -> double[]} hyperbolic cosine. See {@link SimdSupportedUnaryOp#COSH}
 * for the shared VO_MATHLIB performance notes.
 */
public final class SimdLongToDoubleCoshProcessor extends SimdLongToDoubleUnaryProcessor
{
  public SimdLongToDoubleCoshProcessor(ExprVectorProcessor<?> input, DoubleUnivariateLongFunction scalarFallback)
  {
  ...
  protected void processVector(long[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      ...
      va.lanewise(VectorOperators.COSH).intoArray(outValues, i);
    }
    if (i < currentSize) {
      ...
      va.lanewise(VectorOperators.COSH).intoArray(outValues, i, doubleMask);
    }
    if (inputNulls == null) {
