...
/**
 * SIMD specialization of {@code (double[], double[]) -> double[]} maximum. The op is hardcoded to
 * {@link DoubleVector#max} so the JIT statically resolves it to the platform's double-max intrinsic.
 */
public final class SimdDoubleDoubleMaxProcessor extends SimdDoubleDoubleProcessor
{
  public SimdDoubleDoubleMaxProcessor(
  ...
  )
  {
    ...
    if (!hasLeftNulls && !hasRightNulls) {
      for (; i < upperBound; i += laneCount) {
        ...
        va.max(vb).intoArray(outValues, i);
      }
      for (; i < currentSize; i++) {
