...
/**
 * SIMD specialization of {@code (double[], double[]) -> double[]} division. The op is hardcoded to
 * {@link DoubleVector#div} so the JIT statically resolves it to the platform's double-divide intrinsic.
 * Division by zero produces the same {@code +/-Infinity} / {@code NaN} values as the scalar path, no
 * exceptions are thrown for IEEE-754 double division.
 */
public final class SimdDoubleDoubleDivProcessor extends SimdDoubleDoubleProcessor
{
