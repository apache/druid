...
/**
 * SIMD specialization of {@code (long[]) -> double[]} base-10 logarithm. See {@link SimdSupportedUnaryOp#LOG10}
 * for the shared VO_MATHLIB performance notes.
 */
public final class SimdLongToDoubleLog10Processor extends SimdLongToDoubleUnaryProcessor
{
  public SimdLongToDoubleLog10Processor(ExprVectorProcessor<?> input, DoubleUnivariateLongFunction scalarFallback)
  {
  ...
  protected void processVector(long[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      ...
      va.lanewise(VectorOperators.LOG10).intoArray(outValues, i);
    }
    if (i < currentSize) {
      ...
      va.lanewise(VectorOperators.LOG10).intoArray(outValues, i, doubleMask);
    }
    if (inputNulls == null) {
