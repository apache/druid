...
/**
 * SIMD specialization of {@code (long[]) -> double[]} arc tangent. See {@link SimdSupportedUnaryOp#ATAN} for the
 * shared VO_MATHLIB performance notes.
 */
public final class SimdLongToDoubleAtanProcessor extends SimdLongToDoubleUnaryProcessor
{
  public SimdLongToDoubleAtanProcessor(ExprVectorProcessor<?> input, DoubleUnivariateLongFunction scalarFallback)
  {
  ...
  protected void processVector(long[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      ...
      va.lanewise(VectorOperators.ATAN).intoArray(outValues, i);
    }
    if (i < currentSize) {
      ...
      va.lanewise(VectorOperators.ATAN).intoArray(outValues, i, doubleMask);
    }
    if (inputNulls == null) {
