...
/**
 * SIMD specialization of {@code (long[]) -> double[]} hyperbolic sine. See {@link SimdSupportedUnaryOp#SINH} for
 * the shared VO_MATHLIB performance notes.
 */
public final class SimdLongToDoubleSinhProcessor extends SimdLongToDoubleUnaryProcessor
{
  public SimdLongToDoubleSinhProcessor(ExprVectorProcessor<?> input, DoubleUnivariateLongFunction scalarFallback)
  {
  ...
  protected void processVector(long[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      ...
      va.lanewise(VectorOperators.SINH).intoArray(outValues, i);
    }
    if (i < currentSize) {
      ...
      va.lanewise(VectorOperators.SINH).intoArray(outValues, i, doubleMask);
    }
    if (inputNulls == null) {
