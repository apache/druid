...
/**
 * SIMD specialization of {@code (long[]) -> double[]} tangent. See {@link SimdSupportedUnaryOp#TAN} for the
 * shared VO_MATHLIB performance notes.
 */
public final class SimdLongToDoubleTanProcessor extends SimdLongToDoubleUnaryProcessor
{
  public SimdLongToDoubleTanProcessor(ExprVectorProcessor<?> input, DoubleUnivariateLongFunction scalarFallback)
  {
  ...
  protected void processVector(long[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      ...
      va.lanewise(VectorOperators.TAN).intoArray(outValues, i);
    }
    if (i < currentSize) {
      ...
      va.lanewise(VectorOperators.TAN).intoArray(outValues, i, doubleMask);
    }
    if (inputNulls == null) {
