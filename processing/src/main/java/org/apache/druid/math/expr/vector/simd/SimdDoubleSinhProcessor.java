...
/**
 * SIMD specialization of {@code (double[]) -> double[]} hyperbolic sine. See {@link SimdSupportedUnaryOp#SINH}
 * for the shared VO_MATHLIB performance notes.
 */
public final class SimdDoubleSinhProcessor extends SimdDoubleUnaryProcessor
{
  public SimdDoubleSinhProcessor(ExprVectorProcessor<?> input, DoubleUnivariateDoubleFunction scalarFallback)
  {
  ...
  protected void processVector(double[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      DoubleVector.fromArray(SPECIES, input, i).lanewise(VectorOperators.SINH).intoArray(outValues, i);
    }
    if (i < currentSize) {
      final VectorMask<Double> mask = SPECIES.indexInRange(i, currentSize);
      DoubleVector.fromArray(SPECIES, input, i, mask).lanewise(VectorOperators.SINH).intoArray(outValues, i, mask);
    }
    if (inputNulls == null) {
