...
/**
 * SIMD specialization of {@code (double[]) -> double[]} hyperbolic cosine. See {@link SimdSupportedUnaryOp#COSH}
 * for the shared VO_MATHLIB performance notes.
 */
public final class SimdDoubleCoshProcessor extends SimdDoubleUnaryProcessor
{
  public SimdDoubleCoshProcessor(ExprVectorProcessor<?> input, DoubleUnivariateDoubleFunction scalarFallback)
  {
  ...
  protected void processVector(double[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      DoubleVector.fromArray(SPECIES, input, i).lanewise(VectorOperators.COSH).intoArray(outValues, i);
    }
    if (i < currentSize) {
      final VectorMask<Double> mask = SPECIES.indexInRange(i, currentSize);
      DoubleVector.fromArray(SPECIES, input, i, mask).lanewise(VectorOperators.COSH).intoArray(outValues, i, mask);
    }
    if (inputNulls == null) {
