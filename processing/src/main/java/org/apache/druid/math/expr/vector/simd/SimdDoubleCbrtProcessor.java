...
/**
 * SIMD specialization of {@code (double[]) -> double[]} cube root. See {@link SimdSupportedUnaryOp#CBRT} for the
 * shared VO_MATHLIB performance notes.
 */
public final class SimdDoubleCbrtProcessor extends SimdDoubleUnaryProcessor
{
  public SimdDoubleCbrtProcessor(ExprVectorProcessor<?> input, DoubleUnivariateDoubleFunction scalarFallback)
  {
  ...
  protected void processVector(double[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      DoubleVector.fromArray(SPECIES, input, i).lanewise(VectorOperators.CBRT).intoArray(outValues, i);
    }
    if (i < currentSize) {
      final VectorMask<Double> mask = SPECIES.indexInRange(i, currentSize);
      DoubleVector.fromArray(SPECIES, input, i, mask).lanewise(VectorOperators.CBRT).intoArray(outValues, i, mask);
    }
    if (inputNulls == null) {
