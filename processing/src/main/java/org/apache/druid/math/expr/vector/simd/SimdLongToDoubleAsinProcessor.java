...
/**
 * SIMD specialization of {@code (long[]) -> double[]} arc sine. See {@link SimdSupportedUnaryOp#ASIN} for the
 * shared VO_MATHLIB performance notes.
 */
public final class SimdLongToDoubleAsinProcessor extends SimdLongToDoubleUnaryProcessor
{
  public SimdLongToDoubleAsinProcessor(ExprVectorProcessor<?> input, DoubleUnivariateLongFunction scalarFallback)
  {
  ...
  protected void processVector(long[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      ...
      va.lanewise(VectorOperators.ASIN).intoArray(outValues, i);
    }
    if (i < currentSize) {
      ...
      va.lanewise(VectorOperators.ASIN).intoArray(outValues, i, doubleMask);
    }
    if (inputNulls == null) {
