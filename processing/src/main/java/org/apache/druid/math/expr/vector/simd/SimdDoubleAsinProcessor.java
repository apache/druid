...
/**
 * SIMD specialization of {@code (double[]) -> double[]} arc sine. See {@link SimdSupportedUnaryOp#ASIN} for the
 * shared VO_MATHLIB performance notes.
 */
public final class SimdDoubleAsinProcessor extends SimdDoubleUnaryProcessor
{
  public SimdDoubleAsinProcessor(ExprVectorProcessor<?> input, DoubleUnivariateDoubleFunction scalarFallback)
  {
  ...
  protected void processVector(double[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      DoubleVector.fromArray(SPECIES, input, i).lanewise(VectorOperators.ASIN).intoArray(outValues, i);
    }
    if (i < currentSize) {
      final VectorMask<Double> mask = SPECIES.indexInRange(i, currentSize);
      DoubleVector.fromArray(SPECIES, input, i, mask).lanewise(VectorOperators.ASIN).intoArray(outValues, i, mask);
    }
    if (inputNulls == null) {
