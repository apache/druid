...
/**
 * SIMD specialization of {@code (long[]) -> double[]} natural logarithm. The long input is widened lane-by-lane
 * to a {@link DoubleVector} via {@code castShape} before {@code lanewise(VectorOperators.LOG)}. Backed by a
 * vectorized math library (see {@link SimdSupportedUnaryOp#LOG}).
 */
public final class SimdLongToDoubleLogProcessor extends SimdLongToDoubleUnaryProcessor
{
  public SimdLongToDoubleLogProcessor(ExprVectorProcessor<?> input, DoubleUnivariateLongFunction scalarFallback)
  {
  ...
  protected void processVector(long[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      ...
      va.lanewise(VectorOperators.LOG).intoArray(outValues, i);
    }
    if (i < currentSize) {
      ...
      va.lanewise(VectorOperators.LOG).intoArray(outValues, i, doubleMask);
    }
    if (inputNulls == null) {
