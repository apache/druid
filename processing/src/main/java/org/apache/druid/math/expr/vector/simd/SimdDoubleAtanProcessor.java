...
/**
 * SIMD specialization of {@code (double[]) -> double[]} arc tangent. See {@link SimdSupportedUnaryOp#ATAN} for
 * the shared VO_MATHLIB performance notes.
 */
public final class SimdDoubleAtanProcessor extends SimdDoubleUnaryProcessor
{
  public SimdDoubleAtanProcessor(ExprVectorProcessor<?> input, DoubleUnivariateDoubleFunction scalarFallback)
  {
  ...
  protected void processVector(double[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      DoubleVector.fromArray(SPECIES, input, i).lanewise(VectorOperators.ATAN).intoArray(outValues, i);
    }
    if (i < currentSize) {
      final VectorMask<Double> mask = SPECIES.indexInRange(i, currentSize);
      DoubleVector.fromArray(SPECIES, input, i, mask).lanewise(VectorOperators.ATAN).intoArray(outValues, i, mask);
    }
    if (inputNulls == null) {
