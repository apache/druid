...
/**
 * SIMD specialization of {@code (long[]) -> double[]} cosine. See {@link SimdSupportedUnaryOp#COS} for the
 * shared VO_MATHLIB performance notes.
 */
public final class SimdLongToDoubleCosProcessor extends SimdLongToDoubleUnaryProcessor
{
  public SimdLongToDoubleCosProcessor(ExprVectorProcessor<?> input, DoubleUnivariateLongFunction scalarFallback)
  {
  ...
  protected void processVector(long[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      ...
      va.lanewise(VectorOperators.COS).intoArray(outValues, i);
    }
    if (i < currentSize) {
      ...
      va.lanewise(VectorOperators.COS).intoArray(outValues, i, doubleMask);
    }
    if (inputNulls == null) {
