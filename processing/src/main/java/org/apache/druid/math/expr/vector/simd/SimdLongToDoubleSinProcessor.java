...
/**
 * SIMD specialization of {@code (long[]) -> double[]} sine. See {@link SimdSupportedUnaryOp#SIN} for the shared
 * VO_MATHLIB performance notes.
 */
public final class SimdLongToDoubleSinProcessor extends SimdLongToDoubleUnaryProcessor
{
  public SimdLongToDoubleSinProcessor(ExprVectorProcessor<?> input, DoubleUnivariateLongFunction scalarFallback)
  {
  ...
  protected void processVector(long[] input, boolean[] inputNulls, int currentSize)
  {
    ...
    for (; i < upperBound; i += laneCount) {
      ...
      va.lanewise(VectorOperators.SIN).intoArray(outValues, i);
    }
    if (i < currentSize) {
      ...
      va.lanewise(VectorOperators.SIN).intoArray(outValues, i, doubleMask);
    }
    if (inputNulls == null) {
