...
public class IndexIONullColumnsCompatibilityTest extends InitializedNullHandlingTest
{
  ...
  private static class V9IndexLoaderExceptEmptyColumns implements IndexLoader
  {
    ...
    public QueryableIndex load(File inDir, ObjectMapper mapper, boolean lazy, SegmentLazyLoadFailCallback loadFailed) throws IOException
    {
      ...
      for (String columnName : cols) {
        ...
        if (lazy) {
          columns.put(columnName, Suppliers.memoize(
              () -> {
                try {
                  return deserializeColumn(mapper, colBuffer, smooshedFiles, segmentBitmapSerdeFactory);
                }
          ...
          ColumnHolder columnHolder = deserializeColumn(mapper, colBuffer, smooshedFiles, segmentBitmapSerdeFactory);
      ...
      if (lazy) {
        columns.put(ColumnHolder.TIME_COLUMN_NAME, Suppliers.memoize(
            () -> {
              try {
                return deserializeColumn(mapper, timeBuffer, smooshedFiles, segmentBitmapSerdeFactory);
              }
              catch (IOException | RuntimeException e) {
                loadFailed.execute();
                throw Throwables.propagate(e);
        ...
        ColumnHolder columnHolder = deserializeColumn(mapper, timeBuffer, smooshedFiles, segmentBitmapSerdeFactory);
    ...
    ) throws IOException
    {
      ...
      // The second last argument should not be null in production,
      // but it can be null in this particular test as we know that it will not be used.
      return serde.read(byteBuffer, columnConfig, smooshedFiles, null, segmentBitmapSerdeFactory);
    }
  }
}
