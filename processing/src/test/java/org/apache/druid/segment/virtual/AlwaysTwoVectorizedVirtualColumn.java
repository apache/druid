...
public class AlwaysTwoVectorizedVirtualColumn implements VirtualColumn
{
  ...
  )
  {
    ...
    {
      private final VectorSizeInspector inspector = factory.getReadableVectorOffset();
  ...
  {
    Assert.assertEquals(outputName, dimensionSpec.getOutputName());
    final IndexedInts[] rowVector = new IndexedInts[factory.getReadableVectorOffset().getMaxVectorSize()];
  ...
  )
  {
    ...
    final long[] longs = new long[factory.getReadableVectorOffset().getMaxVectorSize()];
    final double[] doubles = new double[factory.getReadableVectorOffset().getMaxVectorSize()];
    final float[] floats = new float[factory.getReadableVectorOffset().getMaxVectorSize()];
    ...
    return new VectorValueSelector()
    {
      ...
      public int getMaxVectorSize()
      {
        return factory.getReadableVectorOffset().getMaxVectorSize();
      }

      @Override
      public int getCurrentVectorSize()
      {
        return factory.getReadableVectorOffset().getCurrentVectorSize();
      }
    };
  }
  ...
  {
    Assert.assertEquals(outputName, columnName);
    final Object[] objects = new Object[factory.getReadableVectorOffset().getMaxVectorSize()];
