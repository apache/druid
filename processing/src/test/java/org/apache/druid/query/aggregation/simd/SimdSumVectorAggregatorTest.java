...
public class SimdSumVectorAggregatorTest extends InitializedNullHandlingTest
{
  ...
  private static void runLong(int size, NullPattern pattern)
  {
    ...
    final FakeVectorValueSelector selector = new FakeVectorValueSelector(size, values, null, null, nulls);
    final String msg = StringUtils.format("type[long] size[%s] nulls[%s]", size, pattern);
  ...
  private static void runDouble(int size, NullPattern pattern)
  {
    ...
    final boolean[] nulls = pattern.toMask(size);
    final FakeVectorValueSelector selector = new FakeVectorValueSelector(size, null, values, null, nulls);
    final String msg = StringUtils.format("type[double] size[%s] nulls[%s]", size, pattern);
    ...
    if (nulls == null) {
      ...
      Assert.assertEquals(msg + " (anyNonNull)", anyNonNull, reported);
      if (reported) {
        Assert.assertEquals(msg, expected, simdBuf.getDouble(0), Math.max(Math.abs(expected) * 1e-12, 1e-12));
      }
    }
  }
  ...
  private static void runFloat(int size, NullPattern pattern)
  {
    ...
    final boolean[] nulls = pattern.toMask(size);
    final FakeVectorValueSelector selector = new FakeVectorValueSelector(size, null, null, values, nulls);
    final String msg = StringUtils.format("type[float] size[%s] nulls[%s]", size, pattern);
    ...
    if (nulls == null) {
      ...
      Assert.assertEquals(
          msg,
          scalarBuf.getFloat(0),
          simdBuf.getFloat(0),
          Math.max(Math.abs(scalarBuf.getFloat(0)) * 1e-5f, 1e-5f)
      );
    } else {
      float expected = 0;
      boolean anyNonNull = false;
      ...
      Assert.assertEquals(msg + " (anyNonNull)", anyNonNull, reported);
      if (reported) {
        Assert.assertEquals(msg, expected, simdBuf.getFloat(0), Math.max(Math.abs(expected) * 1e-5f, 1e-5f));
      }
    }
  }
