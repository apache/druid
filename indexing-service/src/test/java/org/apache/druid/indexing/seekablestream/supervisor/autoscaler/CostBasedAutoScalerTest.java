...
public class CostBasedAutoScalerTest
{
  ...
  {
    Assert.assertEquals(config, autoScaler.getConfig());
  }
  ...
  @Test
  public void testComputeOptimalTaskCountEmptyMetrics()
  {
  ...
  @Test
  public void testComputeOptimalTaskCountZeroPartitions()
  {
  ...
  @Test
  public void testComputeOptimalTask_lowLagScenario()
  {
    // Low lag scenario with low idle - algorithm considers task distribution
    CostMetrics metrics = new CostMetrics(
        System.currentTimeMillis(),
        40.0,  // avgPartitionLag - low
        25,    // currentTaskCount
        100,    // partitionCount
        0.1    // pollIdleRatio - low idle
    );

    int result = autoScaler.computeOptimalTaskCount(Collections.singletonList(metrics));
    // With low lag, algorithm should find a reasonable task count
    Assert.assertTrue("Should return valid task count", result > 0 && result <= 100);
  }

  @Test
  public void testComputeOptimalTask_findsOptimalFromCandidates()
  {
    CostMetrics metrics = new CostMetrics(
        System.currentTimeMillis(),
        5.0,  // avgPartitionLag - very low
        25,    // currentTaskCount
        100,    // partitionCount
        0.01    // pollIdleRatio - very low idle
    );

    int result = autoScaler.computeOptimalTaskCount(Collections.singletonList(metrics));
    System.err.println(result);
    // Algorithm should find an optimal task count
    Assert.assertTrue("Should return valid task count", result > 0 && result <= 100);
  }
