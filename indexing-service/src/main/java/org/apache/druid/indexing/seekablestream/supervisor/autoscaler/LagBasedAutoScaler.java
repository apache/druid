...
public class LagBasedAutoScaler implements SupervisorTaskAutoScaler
{
  ...
  )
  {
    ...
    // Partition counts *can* change without a new instance of this class being created
    this.partitionFactors = factorize(supervisor.getPartitionCount());
  }
  ...
  int computeDesiredTaskCount(List<Long> lags)
  {
    ...
    if (partitionCount <= 0) {
      ...
      return -1;
    } else if (partitionCount != partitionFactors.get(partitionFactors.size() - 1)) { // if partitions were changed
  ...
  private int applyMinMaxChecks(int desiredActiveTaskCount, int currentActiveTaskCount, int partitionCount)
  {
    ...
    if (desiredActiveTaskCount > currentActiveTaskCount && lagBasedAutoScalerConfig.getUseNearestFactorScaling()) {
      desiredActiveTaskCount = nearestFactor(desiredActiveTaskCount);
    }
  ...
  /*
    Finds the nearest divisor of the topic's partition count.
    For now, this only is meant to be used in the scale out case.
    We always find the factor >= desiredTaskCount up to a maximum of partition count.
  */
  private int nearestFactor(int desiredTaskCount)
  {
    int lo = 0;
    ...
    final int factor = partitionFactors.get(Math.min(lo, partitionFactors.size() - 1));
    log.info("Given desiredTaskCount=[%d], found nearest task count=[%d]", desiredTaskCount, factor);
    return factor;
  }
