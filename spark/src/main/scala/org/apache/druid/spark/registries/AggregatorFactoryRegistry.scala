...
object AggregatorFactoryRegistry {
  /**
    ...
    * @param factoryClass The AggregatorFactory class to use when constructing aggregators for type NAME
    */
  def register(name: String, factoryClass: Class[_ <: AggregatorFactory]): Unit = {
    // Cheat
    MAPPER.registerSubtypes(new NamedType(factoryClass, name))
  }
}
