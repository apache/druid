...
object SparkUdfs {
  val bucketRow: UserDefinedFunction =
    udf[Long, AnyRef, String, String]((time: AnyRef, tsFormat: String, granularityStr: String) => {
      val parser = TimestampParser.createObjectTimestampParser(tsFormat)
      val granularity = Granularity.fromString(granularityStr)
      granularity.bucketStart(parser.apply(time)).getMillis
    })
}
