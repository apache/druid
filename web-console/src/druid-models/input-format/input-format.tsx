...
function generateInputFormatFields(streamingType: 'kafka' | 'kinesis' | undefined) {
  const fields = compact([
    {
      ...
      type: 'string',
      suggestions: compact([
        streamingType === 'kafka' ? 'kafka' : undefined,
        'json',
        'csv',
        'tsv',
        'parquet',
  ...
  if (streamingType === 'kafka') {
    fields.push(
        ...
        info: (
          <>
            <p>
              ...
              supported by Jackson library. Those features will be applied when parsing the input
              JSON data.
            </p>
            <p>
              Example:{' '}
      ...
      },
      {
        ...
        info: (
          <>
            ...
            <p>
              If the input is known to be newline delimited JSON (each individual JSON event is
              contained in a single line, separated by newlines), setting this option to true allows
              for more flexible parsing exception handling. Only the lines with invalid JSON syntax
              will be discarded, while lines containing valid JSON events will still be ingested.
            </p>
          </>
        ),
      },
      ...
      },
      {
        ...
        suggestions: ['\x01', '\x00'],
        defined: inputFormat =>
          oneOf(deepGet(inputFormat, 'valueFormat.type'), 'csv', 'tsv', 'regex'),
        info: <>A custom delimiter for multi-value dimensions.</>,
      },
      {
        ...
        defined: inputFormat =>
          oneOf(
            deepGet(inputFormat, 'valueFormat.type'),
            'parquet',
            'orc',
            'avro_ocf',
            'avro_stream',
        ...
        info: (
          <>
            Specifies if the binary column which is not logically marked as a string should be
            treated as a UTF-8 encoded string.
          </>
        ),
      },
      ...
      },
      {
        ...
        info: (
          <>
            Only the value of the first column will be read, the name of the column will be ignored
            so enter anything here.
          </>
        ),
      },
      {
        ...
        suggestions: ['\x01', '\x00'],
        defined: inputFormat =>
          oneOf(deepGet(inputFormat, 'keyFormat.type'), 'csv', 'tsv', 'regex'),
        info: <>A custom delimiter for multi-value dimensions.</>,
      },
      {
...
export const BATCH_INPUT_FORMAT_FIELDS: Field<InputFormat>[] = generateInputFormatFields(undefined);
export const KINESIS_INPUT_FORMAT_FIELDS: Field<InputFormat>[] =
  generateInputFormatFields('kinesis');
export const KAFKA_INPUT_FORMAT_FIELDS: Field<InputFormat>[] = generateInputFormatFields('kafka');
