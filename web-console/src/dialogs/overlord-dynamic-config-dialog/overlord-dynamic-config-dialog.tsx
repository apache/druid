...
export const OverlordDynamicConfigDialog = React.memo(function OverlordDynamicConfigDialog(
  ...
  const [historyRecordsState] = useQueryManager<null, any[]>({
    processQuery: async () => {
      const historyResp = await Api.get(`/druid/indexer/v1/worker/history?count=100`);
  ...
  useQueryManager<null, Record<string, any>>({
    processQuery: async () => {
      try {
        const configResp = await Api.get(`/druid/indexer/v1/worker`);
  ...
  async function saveConfig(comment: string) {
    try {
      await Api.post(API_ENDPOINTS.worker, dynamicConfig, {
        headers: {
