...
public abstract class SQLMetadataConnector implements MetadataStorageConnector
{
	...
	public void createTable(final IDBI dbi, final String tableName, final Iterable<String> sql)
	{
		try {
			...
			log.warn(e, "Exception creating table : DBI : "+dbi+" Table Name : "+tableName+" SQL : "+sql);
		}
	}
					...
					ImmutableList.of(
							String.format(
									"CREATE TABLE %1$s (\n"
											...
											+ "  start VARCHAR(255) NOT NULL,\n"
											+ "  [end] VARCHAR(255) NOT NULL,\n"
											+ "  partitioned numeric(3) NOT NULL,\n"
											+ "  version VARCHAR(255) NOT NULL,\n"
											+ "  used numeric(1) NOT NULL,\n"
					...
					ImmutableList.of(
							String.format(
									"CREATE TABLE %1$s (\n"
											...
											+ "  datasource VARCHAR(255) NOT NULL,\n"
											+ "  payload %2$s NOT NULL,\n"
											+ "  status_payload VARCHAR(8000) NOT NULL,\n"
											+ "  active numeric(1) NOT NULL DEFAULT FALSE,\n"
											+ "  PRIMARY KEY (id)\n"
											+ ")",
											tableName, getPayloadType()
									),
	...
	public static Boolean isSQLServer(final IDBI dbi)
	{
		return dbi.withHandle(
				new HandleCallback<Boolean>()
				{
					...
					{
						return isSQLServer(handle);
					}
				}
				);
	}

	protected static Boolean isSQLServer(final Handle handle) throws SQLException
	{
		log.info("handle.getConnection().getMetaData().getDatabaseProductName() : " + handle.getConnection().getMetaData().getDatabaseProductName());

		return handle.getConnection().getMetaData().getDatabaseProductName().contains("SQL Server");
	}
