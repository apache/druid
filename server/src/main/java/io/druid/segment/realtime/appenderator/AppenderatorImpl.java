...
public class AppenderatorImpl implements Appenderator
{
  ...
  {
    config.getBasePersistDirectory().mkdirs();
  ...
  public int getRowCount(final SegmentIdentifier identifier)
  {
    ...
    for (FireHydrant hydrant : sink) {
      final IncrementalIndex index = hydrant.getIndex();
      if (index != null) {
        retVal += index.size();
      } else {
        retVal += hydrant.getSegment().asQueryableIndex().getNumRows();
      }
    }
  ...
  private Sink getSink(final SegmentIdentifier identifier)
  {
    ...
    if (retVal == null) {
      retVal = new Sink(
          ...
          schema,
          config.withShardSpec(identifier.getShardSpec()),
  ...
  @Override
  public ListenableFuture<SegmentsAndMetadata> pushAll(Committer committer)
  {
    return push(Lists.newArrayList(sinks.keySet()), committer);
  }
  ...
  {
    // TODO: Convert old sink directories to new ones (assuming shardSpec is known upfront)?
    ...
    for (File sinkDir : files) {
      ...
      try {
        ...
        Sink currSink = new Sink(identifier.getInterval(), schema, config, identifier.getVersion(), hydrants);
  ...
  {
    return new File(config.getBasePersistDirectory(), "commit.json");
  }
  ...
  {
    return new File(config.getBasePersistDirectory(), identifier.getIdentifierAsString());
  }
