...
public class DruidConnectionStateListener extends AbstractMonitor implements ConnectionStateListener
{
  ...
  public void stateChanged(CuratorFramework curatorFramework, ConnectionState newState)
  {
    if (newState.isConnected()) {
      ...
      if (disconnectDuration != NIL) {
        emitter.emit(ServiceMetricEvent.builder().setMetricAndValue(METRIC_RECONNECT_TIME, disconnectDuration));
      }
    } else {
      synchronized (this) {
        currentState = newState;
  ...
  {
    emitter.emit(ServiceMetricEvent.builder().setMetricAndValue(METRIC_IS_CONNECTED, isConnected() ? 1 : 0));
    return true;
  }
}
