...
public class SQLTableManager implements TableDefnManager
{
  ...
  @Override
  public long create(TableSpec table) throws DuplicateKeyException
  {
    try {
  ...
  @Override
  public TableSpec read(TableId id)
  {
    return dbi.withHandle(
        new HandleCallback<TableSpec>()
        {
          ...
          public TableSpec withHandle(Handle handle)
          {
            Query<Map<String, Object>> query = handle.createQuery(
                ...
                .bind("name", id.name());
            final ResultIterator<TableSpec> resultIterator =
                query.map((index, r, ctx) ->
                  new TableSpec(
                      ...
                      TableDefn.fromBytes(jsonMapper, r.getBytes(5))
                  ))
                .iterator();
  ...
  @Override
  public long updateDefn(TableId id, TableDefn defn, long oldVersion) throws OutOfDateException
  {
    try {
      return dbi.withHandle(
          new HandleCallback<Long>()
  ...
  @Override
  public long updateDefn(TableId id, TableDefn defn) throws NotFoundException
  {
    try {
      return dbi.withHandle(
          new HandleCallback<Long>()
  ...
  @Override
  public List<TableSpec> listDetails(String dbSchema)
  {
    return dbi.withHandle(
            ...
            final ResultIterator<TableSpec> resultIterator =
                query.map((index, r, ctx) ->
                  new TableSpec(
                      ...
                      TableDefn.fromBytes(jsonMapper, r.getBytes(6))))
                .iterator();
            return Lists.newArrayList(resultIterator);
          }
        }
  ...
  protected synchronized void sendAddition(TableSpec table, long updateTime)
  {
    if (listeners.isEmpty()) {
      return;
    }
    TableSpec newTable = table.fromInsert(table.dbSchema(), updateTime);
  ...
  protected synchronized void sendUpdate(TableId id)
  {
    ...
    TableSpec updatedTable = read(id);
    for (Listener listener : listeners) {
