...
public class DataSourcesResource
{
  ...
  )
  {
    try {
      ...
      // When coordinatingVersions is configured, additionally verify per-version coverage.
      ...
      if (!coordinatingVersions.isEmpty() && matchingLoadRule != null) {
        ...
        for (Map.Entry<String, Integer> tierEntry : matchingLoadRule.getTieredReplicants().entrySet()) {
          ...
          final Set<String> activeVersions =
              activeVersionsByTier.getOrDefault(tierEntry.getKey(), Set.of());
          for (String activeVersion : activeVersions) {
            if (!isSegmentLoadedForVersion(servedSegmentsInInterval, descriptor, tierEntry.getKey(), activeVersion)) {
              return Response.ok(false).build();
            }
          }
        }
  ...
  /**
   * Builds a {@code tier -> activeVersions} map in a single pass over the server inventory. Only
   * segment-replication-target servers whose {@code version} is in
   * {@code coordinatingVersions} contribute. Tiers with no active versions are absent from the
   * returned map. Versions with no online servers do not block handoff.
   */
  private Map<String, Set<String>> computeActiveVersionsByTier(Set<String> coordinatingVersions)
  {
    final Map<String, Set<String>> activeVersionsByTier = new HashMap<>();
    for (DruidServer server : serverInventoryView.getInventory()) {
      if (!server.getType().isSegmentReplicationTarget()) {
        continue;
      }
      final String version = server.getMetadata().getVersion();
      if (version == null || !coordinatingVersions.contains(version)) {
        continue;
      }
      activeVersionsByTier.computeIfAbsent(server.getTier(), t -> new HashSet<>()).add(version);
    }
    return activeVersionsByTier;
  }

  /**
   * Returns true if at least one segment-replication-target server in the given {@code tier} and
   * {@code version} serves the segment described by {@code descriptor}. The tier check is required
   * because the same {@code version} name may appear across multiple tiers, so a server
   * outside the rule-required tier must not satisfy the per-tier handoff check.
   */
  static boolean isSegmentLoadedForVersion(
