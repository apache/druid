...
public class DruidCoordinator
{
  ...
  private class CollectSegmentStats implements CoordinatorDuty
  {
    ...
    public DruidCoordinatorRuntimeParams run(DruidCoordinatorRuntimeParams params)
    {
      ...
      final Stopwatch unavailableTime = Stopwatch.createStarted();
      final Object2IntMap<String> dsToUnavailable = getDatasourceToUnavailableSegmentCount();
      ...
      unavailableTime.stop();

      final Stopwatch underReplTime = Stopwatch.createStarted();
      final Map<String, Object2LongMap<String>> tierToDsToUnderRepl = getTierToDatasourceToUnderReplicatedCount(false);
      ...
      final Stopwatch deepStorageTime = Stopwatch.createStarted();
      final Object2IntMap<String> dsToDeepStorageOnly = getDatasourceToDeepStorageQueryOnlySegmentCount();
      ...
      deepStorageTime.stop();

      log.info(
          "CollectSegmentStats summary: unavailableTotal[%,d] across[%d] ds,"
          + " underReplicatedTotal[%,d] across[%d] tiers, deepStorageOnly[%,d] across[%d] ds;"
          + " unavailableMs[%,d], underReplicatedMs[%,d], deepStorageMs[%,d], totalMs[%,d].",
          unavailableTotal, dsToUnavailable.size(),
          underReplTotal, tierCount,
          deepStorageTotal, dsToDeepStorageOnly.size(),
          unavailableTime.millisElapsed(), underReplTime.millisElapsed(),
          deepStorageTime.millisElapsed(), totalTime.millisElapsed()
      );
