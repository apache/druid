...
public class FailureCountServerSelectorStrategy implements ServerSelectorStrategy
{
  private static final Comparator<QueryableDruidServer> COMPARATOR =
      Comparator.comparingInt(s -> {
        DirectDruidClient client = (DirectDruidClient) s.getQueryRunner();
        return client.getNumOpenConnections() + client.getNumFailedConnections();
      });
