...
public class ClosedSegmentsAppendableSegmentsBatchAppenderatorTest extends InitializedNullHandlingTest
{
  ...
  {
    try (final ClosedSegmentsAppendableSegmentsBatchAppenderatorTester tester = new ClosedSegmentsAppendableSegmentsBatchAppenderatorTester(3, false)) {
      ...
      // getDataSource
      Assert.assertEquals(ClosedSegmentsAppendableSegmentsBatchAppenderatorTester.DATASOURCE, appenderator.getDataSource());
      ...
      // since we just added three rows and the max rows in memory is three, all the segments (appendable seg etc.)
  ...
  {
    try (final ClosedSegmentsAppendableSegmentsBatchAppenderatorTester tester = new ClosedSegmentsAppendableSegmentsBatchAppenderatorTester(3, true)) {
      final Appenderator appenderator = tester.getAppenderator();
      ...
      // since we just added three rows and the max rows in memory is three, all the segments (appendable segs etc.)
      // above should be cleared now
      Assert.assertEquals(
          Collections.emptyList(),
          ((BatchAppenderator) appenderator).getInMemorySegments().stream().sorted().collect(Collectors.toList())
  ...
  {
    try (final ClosedSegmentsAppendableSegmentsBatchAppenderatorTester tester = new ClosedSegmentsAppendableSegmentsBatchAppenderatorTester(1, false)) {
      final Appenderator appenderator = tester.getAppenderator();
      ...
      // since we just added one row and the max rows in memory is one, all the segments (appendable segs etc)
      // above should be cleared now
      Assert.assertEquals(
          Collections.emptyList(),
          ((BatchAppenderator) appenderator).getInMemorySegments().stream().sorted().collect(Collectors.toList())
  ...
  {
    try (
        final ClosedSegmentsAppendableSegmentsBatchAppenderatorTester tester = new ClosedSegmentsAppendableSegmentsBatchAppenderatorTester(
            100,
            1024,
            null,
            true,
  ...
  {
    try (final ClosedSegmentsAppendableSegmentsBatchAppenderatorTester tester = new ClosedSegmentsAppendableSegmentsBatchAppenderatorTester(100, 15000, true)) {
      final Appenderator appenderator = tester.getAppenderator();
      ...
      int appendableSegmentSizeOverhead = BatchAppenderator.ROUGH_OVERHEAD_PER_APPENDABLE_SEGMENT;
      // currPartialSegment in the appendable segment still has > 0 bytesInMemory since we do not persist yet
      Assert.assertEquals(
      ...
      // We do multiple more adds to the same appendable segment to cause persist.
      for (int i = 0; i < 53; i++) {
      ...
      // currPartialSegment in the appendable segment has 0 bytesInMemory since we just did a persist
      Assert.assertEquals(
          currentInMemoryIndexSize,
          ((BatchAppenderator) appenderator).getBytesInMemory(IDENTIFIERS.get(0))
      );
      // no appendable segments no partial segments after a persist, so we should have zero bytes currently in memory
      Assert.assertEquals(
          currentInMemoryIndexSize,
          ((BatchAppenderator) appenderator).getBytesCurrentlyInMemory()
      );
      ...
      currentInMemoryIndexSize = 0;
      // We are now over maxSizeInBytes after the add. Hence, we do a persist.
      // so no appendable segments & partial segments should be in memory...
      Assert.assertEquals(
          currentInMemoryIndexSize,
          ((BatchAppenderator) appenderator).getBytesInMemory(IDENTIFIERS.get(0))
      );
  ...
  {
    try (final ClosedSegmentsAppendableSegmentsBatchAppenderatorTester tester =
             new ClosedSegmentsAppendableSegmentsBatchAppenderatorTester(100, 5180, true)) {
      final Appenderator appenderator = tester.getAppenderator();
      appenderator.startJob();
      appenderator.add(IDENTIFIERS.get(0), createInputRow("2000", "foo", 1), null);
    }
  ...
  {
    try (final ClosedSegmentsAppendableSegmentsBatchAppenderatorTester tester =
             new ClosedSegmentsAppendableSegmentsBatchAppenderatorTester(100, 10000, true)) {
      final Appenderator appenderator = tester.getAppenderator();
  ...
  {
    try (final ClosedSegmentsAppendableSegmentsBatchAppenderatorTester tester =
             new ClosedSegmentsAppendableSegmentsBatchAppenderatorTester(1000, 28748, true)) {
      final Appenderator appenderator = tester.getAppenderator();
      ...
      int nullHandlingOverhead = NullHandling.sqlCompatible() ? 1 : 0;
      int currentInMemoryIndexSize = 182 + nullHandlingOverhead;
      int appendableSegmentSizeOverhead = 2 * BatchAppenderator.ROUGH_OVERHEAD_PER_APPENDABLE_SEGMENT;
      // currPartialSegment in the appendable segment still has > 0 bytesInMemory since we do not persist yet
      Assert.assertEquals(
          currentInMemoryIndexSize,
          ((BatchAppenderator) appenderator).getBytesInMemory(IDENTIFIERS.get(0))
      ...
      // appendable segments + currPartialSegment size is 0 since we just persist all indexes to disk.
      currentInMemoryIndexSize = 0;
      // We are now over maxSizeInBytes after the add. Hence, we do a persist.
      // currPartialSegment and the appendable segment has 0 bytesInMemory since we just did a persist
      Assert.assertEquals(
          currentInMemoryIndexSize,
          ((BatchAppenderator) appenderator).getBytesInMemory(IDENTIFIERS.get(0))
      );
      ...
      // Add a single row after persisted to appendable segment 0
      appenderator.add(IDENTIFIERS.get(0), createInputRow("2000", "bob", 1), null);
      // currPartialSegment in the appendable segment still has > 0 bytesInMemory since we do not persist yet
      currentInMemoryIndexSize = 182 + nullHandlingOverhead;
      Assert.assertEquals(
      ...
      // only one appendable segment so far:
      appendableSegmentSizeOverhead = BatchAppenderator.ROUGH_OVERHEAD_PER_APPENDABLE_SEGMENT;
      Assert.assertEquals(
          currentInMemoryIndexSize + appendableSegmentSizeOverhead,
          ((BatchAppenderator) appenderator).getBytesCurrentlyInMemory()
      );
      // Now add a single row to appendable segment 1
      ...
      // We do multiple more adds to the both appendable segment to cause persist.
      for (int i = 0; i < 49; i++) {
        // 186 bytes
        appenderator.add(IDENTIFIERS.get(0), createInputRow("2000", "bar_" + i, 1), null);
        appenderator.add(IDENTIFIERS.get(1), createInputRow("2000", "bar_" + i, 1), null);
  ...
  {
    try (final ClosedSegmentsAppendableSegmentsBatchAppenderatorTester tester =
             new ClosedSegmentsAppendableSegmentsBatchAppenderatorTester(100, -1, true)) {
      final Appenderator appenderator = tester.getAppenderator();
  ...
  @Test
  public void testAllPartialSegmentsAreRecovered() throws Exception
  {
    try (final ClosedSegmentsAppendableSegmentsBatchAppenderatorTester tester = new ClosedSegmentsAppendableSegmentsBatchAppenderatorTester(1, false)) {
      final Appenderator appenderator = tester.getAppenderator();
      ...
      // Since maxRowsInMemory is one there ought to be three partial segments stored and recovered
  ...
  public void testTotalRowsPerSegment() throws Exception
  {
    try (final ClosedSegmentsAppendableSegmentsBatchAppenderatorTester tester = new ClosedSegmentsAppendableSegmentsBatchAppenderatorTester(3, false)) {
      ...
      // total rows per segment ought to be preserved even when appendable segments are removed from memory:
      addResult1 =
          appenderator.add(IDENTIFIERS.get(1), createInputRow("2000", "bat", 1), null);
      Assert.assertEquals(1, ((BatchAppenderator) appenderator).getRowsInMemory());
  ...
  {
    final ClosedSegmentsAppendableSegmentsBatchAppenderatorTester tester = new ClosedSegmentsAppendableSegmentsBatchAppenderatorTester(2, false);
    final Appenderator appenderator = tester.getAppenderator();
  ...
  public void testVerifyRowIngestionMetrics() throws Exception
  {
    ...
    try (final ClosedSegmentsAppendableSegmentsBatchAppenderatorTester tester =
             new ClosedSegmentsAppendableSegmentsBatchAppenderatorTester(5,
                                                                         10000L,
  ...
  public void testPushContract() throws Exception
  {
    final RowIngestionMeters rowIngestionMeters = new SimpleRowIngestionMeters();
    try (final ClosedSegmentsAppendableSegmentsBatchAppenderatorTester tester =
             new ClosedSegmentsAppendableSegmentsBatchAppenderatorTester(1,
                                                                         50000L,
                                                                         null, false, rowIngestionMeters
             )) {
      final Appenderator appenderator = tester.getAppenderator();
  ...
  private static SegmentIdWithShardSpec createNonUTCSegmentId(String interval, String version, int partitionNum)
  {
    return new SegmentIdWithShardSpec(
        ClosedSegmentsAppendableSegmentsBatchAppenderatorTester.DATASOURCE,
