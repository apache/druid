...
public class ConnectionCountServerSelectorStrategyTest
{
  ...
  public void testDifferentConnectionCount()
  {
    ...
    for (int i = 0; i < 100; ++i) {
      Assert.assertEquals(s2, serverSelector.pick(null, CloneQueryMode.EXCLUDE));
    }
  }
  ...
  public void testBalancerTieBreaking()
  {
    ...
    for (int i = 0; i < 100; ++i) {
      pickedServers.add(serverSelector.pick(null, CloneQueryMode.EXCLUDE).getServer().getName());
    }
    Assert.assertTrue(
