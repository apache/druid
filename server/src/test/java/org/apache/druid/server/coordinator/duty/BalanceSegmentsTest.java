...
public class BalanceSegmentsTest
{
  ...
  @Test
  public void testMaxSegmentsToMoveIsHonoredAcrossVersions()
  {
  ...
  @Test
  public void testUnusedVersionBalanceBudgetCanBeUsedByLaterVersions()
  {
    final ServerHolder redHolder1 = createHolder(createHistorical("red1", "red"));
    final ServerHolder redHolder2 = createHolder(createHistorical("red2", "red"));
    final ServerHolder blueHolder1 = createHolder(createHistorical("blue1", "blue"), segment1, segment2);
    final ServerHolder blueHolder2 = createHolder(createHistorical("blue2", "blue"));
    ...
    Assert.assertEquals(1L, totalMoved);
    Assert.assertEquals(1, blueHolder2.getPeon().getSegmentsToLoad().size());
  }
  ...
  @Test
  public void testBalancerDoesNotMoveSegmentsAcrossVersions()
  {
    // Two red servers (one full, one empty) and two blue servers (one full, one empty).
    // With coordinatingVersions configured, balancing must stay within each version:
  ...
  private DruidServer createHistorical(String name, String version)
  {
    return new DruidServer(
        new DruidServerMetadata(name, name, null, 100L, null, ServerType.HISTORICAL, "normal", 0, version)
    );
  }
