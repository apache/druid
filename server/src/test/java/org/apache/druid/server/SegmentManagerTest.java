...
public class SegmentManagerTest
{
  private static final SegmentLoader SEGMENT_LOADER = new SegmentLoader()
  {
    ...
    @Override
    public Segment getSegment(final DataSegment segment, boolean lazy, CommonCallback loadFailed)
    {
  ...
  public void testDropSegment() throws SegmentLoadingException, ExecutionException, InterruptedException
  {
    for (DataSegment eachSegment : SEGMENTS) {
      Assert.assertTrue(segmentManager.loadSegment(eachSegment, false, CommonCallback.NOOP));
    }
  ...
  {
    Assert.assertTrue(segmentManager.loadSegment(SEGMENTS.get(0), false, CommonCallback.NOOP));
    Assert.assertTrue(segmentManager.loadSegment(SEGMENTS.get(2), false, CommonCallback.NOOP));
  ...
  public void testLoadDuplicatedSegmentsSequentially() throws SegmentLoadingException
  {
    ...
    Assert.assertFalse(segmentManager.loadSegment(SEGMENTS.get(0), false, CommonCallback.NOOP));
  ...
  {
    segmentManager.loadSegment(SEGMENTS.get(0), false, CommonCallback.NOOP);
    ...
    for (DataSegment segment : expectedExistingSegments) {
      ...
      expectedTimeline.add(
          ...
          segment.getShardSpec().createChunk(
              ReferenceCountingSegment.wrapSegment(SEGMENT_LOADER.getSegment(segment, false, CommonCallback.NOOP), segment.getShardSpec())
          )
      );
    }
