...
public class BalancerStrategyTest
{
  ...
  public void findNewSegmentHomeReplicatorNotEnoughSpace()
  {
    final ServerHolder serverHolder = new ServerHolder(
        new DruidServer("server1", "host1", null, 10L, ServerType.HISTORICAL, DruidServer.DEFAULT_TIER, 0, DruidServer.DEFAULT_GUILD).addDataSegment(proposedDataSegment).toImmutableDruidServer(),
  ...
  public void findNewSegmentHomeReplicatorNotEnoughNodesForReplication()
  {
    final ServerHolder serverHolder1 = new ServerHolder(
        new DruidServer("server1", "host1", null, 1000L, ServerType.HISTORICAL, DruidServer.DEFAULT_TIER, 0, DruidServer.DEFAULT_GUILD).addDataSegment(proposedDataSegment).toImmutableDruidServer(),
        new LoadQueuePeonTester());

    final ServerHolder serverHolder2 = new ServerHolder(
        new DruidServer("server2", "host2", null, 1000L, ServerType.HISTORICAL, DruidServer.DEFAULT_TIER, 0, DruidServer.DEFAULT_GUILD).addDataSegment(proposedDataSegment).toImmutableDruidServer(),
        new LoadQueuePeonTester());
  ...
  {
    final ServerHolder serverHolder = new ServerHolder(
        new DruidServer("server1", "host1", null, 1000L, ServerType.HISTORICAL, DruidServer.DEFAULT_TIER, 0, DruidServer.DEFAULT_GUILD).toImmutableDruidServer(),
        new LoadQueuePeonTester());
    serverHolders = new ArrayList<>();
    serverHolders.add(serverHolder);
    final ServerHolder foundServerHolder = balancerStrategy.findNewSegmentHomeReplicator(proposedDataSegment, serverHolders);
