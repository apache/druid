 ...
 * <p>
 * A historical acknowledges a load or drop over HTTP long before the Coordinator's inventory view reflects it. If the
 * Coordinator forgets the operation at ack time, there is a window in which the operation is visible in neither the
 * load queue nor the inventory, and the replica looks plainly loaded. During a move that reads as over-replication,
 * and the Coordinator drops what is in fact the last remaining replica.
 * <p>
 * These tests reproduce that window directly: the server acks, but the inventory is not advanced.
 */
public class SegmentMoveRaceTest
{
  ...
  public void setUp()
  {
    exec = MoreExecutors.listeningDecorator(Execs.multiThreaded(1, "SegmentMoveRaceTest-%d"));
    balancerStrategy = new CostBalancerStrategy(exec);
    loadQueueManager = new SegmentLoadQueueManager(null, new AlwaysConfirmingTaskMaster());
  ...
  public void testNoDropIsQueuedWhileTheSourceDropAwaitsInventoryConfirmation()
  {
    ...
    // The move target acks and its inventory catches up, so B is now genuinely serving the segment, and the drop is
    // queued on the source.
    serverB.ackPendingRequests();
    serverB.confirmInventory(true);
    completeMoveOnceTargetIsServing();
    Assert.assertTrue(serverA.peon.getSegmentsToDrop().contains(segment));

    // The source acks the drop, but its inventory has not caught up: it still reports the segment as loaded. This is
    // exactly the state in the issue's trace between "Server[A] completed request[DROP]" and the next inventory sync.
  ...
  public void testNoDropIsQueuedWhileTheSourceIsStillMarkedToDrop()
  {
    ...
    // The move target acks and the inventory catches up, but the source has not been told to drop yet.
    serverB.ackPendingRequests();
    serverB.confirmInventory(true);
  ...
  @Test
  public void testSourceIsNotDroppedUntilTheTargetReplicaIsConfirmed()
  {
    moveSegmentFromAtoB();

    // The target acknowledges the load, but Brokers do not know about the new replica until their own sync lands,
    // so the source must keep serving. See apache/druid#18738.
    serverB.ackPendingRequests();
    completeMoveOnceTargetIsServing();
    Assert.assertTrue(serverA.peon.getSegmentsToDrop().isEmpty());
    Assert.assertTrue(serverA.peon.getSegmentsMarkedToDrop().contains(segment));

    // Once the inventory confirms the replica on B, the source drop is queued.
    serverB.confirmInventory(true);
    completeMoveOnceTargetIsServing();
    Assert.assertTrue(serverA.peon.getSegmentsToDrop().contains(segment));
    Assert.assertFalse(serverA.peon.getSegmentsMarkedToDrop().contains(segment));
  }

  @Test
  public void testSourceIsDroppedOnAckWhenConfirmMoveBeforeDropIsOff()
  {
    // The escape hatch for operators whose balancing is throttled by holding move sources for an extra run.
    loadQueueManager = new SegmentLoadQueueManager(null, new AlwaysConfirmingTaskMaster(false));

    moveSegmentFromAtoB();
    serverB.ackPendingRequests();

    // No inventory confirmation and no CompletePendingMoves run: the drop is queued straight off the ack.
    Assert.assertTrue(serverA.peon.getSegmentsToDrop().contains(segment));
    Assert.assertFalse(serverA.peon.getSegmentsMarkedToDrop().contains(segment));
  }
  ...
  {
    // The retained-until-confirmed set must stay proportional to load queue depth rather than to how many segments
    // the server holds, or it would be unusable on a cluster with millions of segments per server.
    ...
    // Once the inventory reflects them, they are all retired: the server now holds 200 segments and tracks none.
  ...
  /**
   * Queues the move using the production code path, then advances both peons to the point where the requests have
   * been sent to their servers but not yet acknowledged.
   */
  private void moveSegmentFromAtoB()
  {
    ...
    Assert.assertTrue(loadQueueManager.moveSegment(segment, holderA, holderB));
    Assert.assertTrue(serverA.peon.getSegmentsMarkedToDrop().contains(segment));
  }

  /**
   * Runs the duty that graduates a pending MOVE_FROM into a real drop.
   */
  private void completeMoveOnceTargetIsServing()
  {
    final DruidCluster cluster = buildCluster();
    new CompletePendingMoves(new AlwaysConfirmingTaskMaster()).run(makeRuntimeParams(cluster));
  }
  ...
  /**
   * A {@link LoadQueueTaskMaster} that only needs to answer the one question {@link CompletePendingMoves} and
   * {@link SegmentLoadQueueManager#moveSegment} ask of it.
   */
  private static class AlwaysConfirmingTaskMaster extends LoadQueueTaskMaster
  {
    AlwaysConfirmingTaskMaster()
    {
      this(true);
    }

    AlwaysConfirmingTaskMaster(boolean confirmMoveBeforeDrop)
    {
      super(MAPPER, null, null, new HttpLoadQueuePeonConfig(null, null, null, confirmMoveBeforeDrop), null, null);
    }
  }

  /**
   * A historical paired with a real {@link HttpLoadQueuePeon} whose server responses and inventory updates are driven
  ...
  private class TestServer implements DataSegmentChangeHandler
  {
    ...
    TestServer(String name)
    {
      ...
      this.peon = new HttpLoadQueuePeon(
          ...
          MAPPER,
          new TestHttpClient(),
          new HttpLoadQueuePeonConfig(null, null, 10, true),
    ...
    {
      // Mirrors PrepareBalancerAndLoadQueues: the queue is read against the very inventory snapshot it is paired
      // with, so an operation is reported as pending exactly when this snapshot does not yet reflect it.
    ...
    /**
     * Lets the server receive and acknowledge everything currently queued, and runs the resulting callbacks. The
     * inventory is deliberately left untouched.
     */
    ...
    /**
     * Advances the inventory view to match what the server actually did, the way a segment sync would. The peon
     * finds out on the next {@link #toHolder()}, which is when the Coordinator reads the queue against it.
     */
