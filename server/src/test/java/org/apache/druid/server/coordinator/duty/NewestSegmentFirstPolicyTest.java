...
public class NewestSegmentFirstPolicyTest
{
  ...
  @Test
  public void testIteratorReturnsSegmentsAsSegmentsWasCompactedAndHaveDifferentDimensionExclusions()
  {
    // Same indexSpec as what is set in the auto compaction config
    Map<String, Object> indexSpec = mapper.convertValue(new IndexSpec(), new TypeReference<Map<String, Object>>() {});
    // Same partitionsSpec as what is set in the auto compaction config
    ...
    // Create segments that were compacted (CompactionState != null) and have
    // DimensionExclusions=["foo", "bar"] for interval 2017-10-01T00:00:00/2017-10-02T00:00:00,
    // DimensionExclusions=["foo"] for interval 2017-10-02T00:00:00/2017-10-03T00:00:00,
    // DimensionExclusions=null for interval 2017-10-03T00:00:00/2017-10-04T00:00:00 (DimensionExclusions was not set during last compaction)
    // and dimensionsSpec=null for interval 2017-10-04T00:00:00/2017-10-05T00:00:00 (dimensionsSpec was not set during last compaction)
    final VersionedIntervalTimeline<String, DataSegment> timeline = createTimeline(
        new SegmentGenerateSpec(
            Intervals.of("2017-10-01T00:00:00/2017-10-02T00:00:00"),
            new Period("P1D"),
            null,
            new CompactionState(partitionsSpec, new DimensionsSpec(null, ImmutableList.of("bar", "foo"), null), indexSpec, null)
        ),
        new SegmentGenerateSpec(
            Intervals.of("2017-10-02T00:00:00/2017-10-03T00:00:00"),
            new Period("P1D"),
            null,
            new CompactionState(partitionsSpec, new DimensionsSpec(null, ImmutableList.of("foo"), null), indexSpec, null)
        ),
        new SegmentGenerateSpec(
            Intervals.of("2017-10-03T00:00:00/2017-10-04T00:00:00"),
            new Period("P1D"),
    ...
    // Auto compaction config sets DimensionExclusions=["foo"]
    final CompactionSegmentIterator iterator = policy.reset(
        ImmutableMap.of(DATA_SOURCE, createCompactionConfig(
            130000,
            new Period("P0D"),
            null,
            new UserCompactionTaskDimensionsConfig(null, ImmutableList.of("foo"))
        )),
        ImmutableMap.of(DATA_SOURCE, timeline),
        Collections.emptyMap()
    );
