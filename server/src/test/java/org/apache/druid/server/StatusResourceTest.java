...
public class StatusResourceTest
{
  ...
  public void testStackTrace()
  {
    ...
    Assertions.assertEquals(Thread.currentThread().getState().name(), currentThread.getThreadState());
    ...
    if (JvmUtils.isThreadCpuTimeEnabled()) {
      Assertions.assertNotNull(currentThread.getCpuTimeNs());
      Assertions.assertNotNull(currentThread.getUserCpuTimeNs());
    }
  }
  ...
  public void testStackTraceFormatsWaitingLockOnStackFrame() throws Exception
  {
    ...
    try {
      ...
      Assertions.assertEquals(Thread.State.WAITING.name(), thread.getThreadState());
      Assertions.assertTrue(
  ...
  public void testStackTraceFormatsHeldMonitorAndSynchronizer() throws Exception
  {
    ...
    Assumptions.assumeTrue(threadMxBean.isObjectMonitorUsageSupported());
    Assumptions.assumeTrue(threadMxBean.isSynchronizerUsageSupported());
    ...
    try {
      Assertions.assertTrue(locksHeld.await(5, TimeUnit.SECONDS));
      final StackTraceCollector.ThreadStackTrace thread = new StackTraceCollector().collect()
          .getThreads()
          .stream()
          ...
          .orElse(null);
      Assertions.assertNotNull(thread);
      Assertions.assertTrue(thread.getStackTrace().contains("\t-  locked " + monitor));
      Assertions.assertTrue(thread.getStackTrace().contains("\tNumber of locked synchronizers = 1\n"));
      Assertions.assertTrue(thread.getStackTrace().contains("java.util.concurrent.locks.ReentrantLock$"));
    }
    finally {
