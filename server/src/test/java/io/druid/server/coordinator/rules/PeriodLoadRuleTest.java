...
public class PeriodLoadRuleTest
{
  ...
  public void testAppliesToPeriod()
  {
    ...
    Assert.assertTrue(futureRule.appliesTo(builder.interval(new Interval(now.minusWeeks(1), now)).build(), now));
    Assert.assertTrue(
        futureRule.appliesTo(
            builder.interval(new Interval(now.minusDays(1), now.plusDays(1)))
                   .build(),
            now
        )
    ...
    Assert.assertTrue(
        futureRule.appliesTo(
            builder.interval(new Interval(now, now.plusDays(2)))
                   .build(),
            now
        )
    );
