...
public class KafkaSupervisorTest extends EasyMockSupport
{
  ...
  public static void setupClass() throws Exception
  {
    ...
    kafkaServer.start();
    kafkaHost = StringUtils.safeFormat("localhost:%d", kafkaServer.getPort());
  ...
  public void testStop() throws Exception
  {
    expect(taskMaster.getTaskRunner()).andReturn(Optional.of(taskRunner)).anyTimes();
    taskClient.close();
    taskRunner.unregisterListener(StringUtils.safeFormat("KafkaSupervisor-%s", DATASOURCE));
    replayAll();
        ...
        for (int j = 0; j < numEventsPerPartition; j++) {
          kafkaProducer.send(
              new ProducerRecord<byte[], byte[]>(
                  ...
                  i,
                  null,
                  StringUtils.toUtf8(StringUtils.safeFormat("event-%d", j))
              )
          ).get();
        }
      }
  ...
  private static class TestableKafkaSupervisor extends KafkaSupervisor
  {
    ...
    {
      return StringUtils.safeFormat("sequenceName-%d", groupId);
    }
  }
}
