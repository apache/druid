...
public class BatchDataHighWatermarkFactory extends WatermarkCursorFactory
{
  @Inject
  public BatchDataHighWatermarkFactory(
  ...
  {
    return BatchDataHighWatermark.TYPE;
  }
  ...
  {
    return new BatchDataHighWatermark(
        cache,
        watermarkSink,
        dataSource,
        interval
  ...
  {
    throw new UOE("%s cannot be manually overridden", BatchDataHighWatermark.TYPE);
  }

  @Override
  public Watermark getWatermark(String dataSource)
  {
    return new BatchDataHighWatermark(cache, watermarkSink, dataSource, null);
  }
}
