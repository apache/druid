 ...
 * IncrementalIndex (realtime) and QueryableIndex (historical) segments that were created
 * with HLLSketchBuild aggregator.
 * <p>
 * The fix ensures that both segment types report the same column type (COMPLEX<HLLSketch>)
 * by using the serde's normal type name in IncrementalIndex.
 */
public class HllSketchBuildSegmentMetadataQueryTest extends InitializedNullHandlingTest
{
  private static final String DATA_SOURCE = "test_datasource";
  private static final String SKETCH_COLUMN = "sketch";
  ...
  /**
   * Verifies fix for issue #14315: SegmentMetadataQuery should work correctly when
   * querying across IncrementalIndex (realtime) and QueryableIndex (historical) segments
   * that were created with HLLSketchBuild aggregator.
   * <p>
   * Both segment types should report the same column type (COMPLEX<HLLSketch>) and
   * the merged metadata query should succeed without errors.
   */
  @Test
  public void testSegmentMetadataQueryWithHllSketchBuildAcrossMixedSegments() throws Exception
  {
    final AggregatorFactory[] aggregators = new AggregatorFactory[]{
        new CountAggregatorFactory("count"),
        new HllSketchBuildAggregatorFactory(SKETCH_COLUMN, "id", null, null, null, false, false)
    };

    // Create an IncrementalIndex (simulates realtime/unpersisted segment)
    IncrementalIndex incrementalIndex = createIncrementalIndex(aggregators);
    addRows(incrementalIndex, 0, 100);

    // Create a QueryableIndex (simulates historical/persisted segment)
    IncrementalIndex indexToPersist = createIncrementalIndex(aggregators);
    ...
    // Verify all expected columns are present (4 columns: __time, dim, count, sketch)
    ...
    Assert.assertEquals(
        ColumnType.ofComplex(HllSketchModule.TYPE_NAME),
    ...
    Assert.assertTrue(
        "Sketch aggregator should be HllSketchMergeAggregatorFactory but was " + sketchAggregator.getClass().getName(),
        sketchAggregator instanceof HllSketchMergeAggregatorFactory
    );

    HllSketchMergeAggregatorFactory hllAggregator = (HllSketchMergeAggregatorFactory) sketchAggregator;
    Assert.assertEquals("Aggregator name should match", SKETCH_COLUMN, hllAggregator.getName());
    Assert.assertEquals("Field name should match", SKETCH_COLUMN, hllAggregator.getFieldName());
  ...
  public void testSegmentMetadataQueryWithOnlyPersistedSegments() throws Exception
  {
    ...
    IncrementalIndex index1 = createIncrementalIndex(aggregators);
    ...
    QueryableIndex queryableIndex1 = indexIO.loadIndex(segmentDir1);

    IncrementalIndex index2 = createIncrementalIndex(aggregators);
    ...
    QueryableIndex queryableIndex2 = indexIO.loadIndex(segmentDir2);
    ...
    index1.close();
    index2.close();
    queryableIndex1.close();
    queryableIndex2.close();
  }
