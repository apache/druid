...
public abstract class S3DataSegmentTransferUtility
{
  ...
  // S3 current caps direct upload sizes at 5GB
  private static final long S3_MAX_DIRECT_UPLOAD_BYTES = 5 * (2L << 30);
  ...
  protected DataSegment transfer(DataSegment segment, Map<String, Object> targetLoadSpec, boolean deleteSource)
  ...
  ) throws IOException, SegmentLoadingException
  {
    ...
    if (s3Client.doesObjectExist(s3Bucket, s3Path)) {
      ...
      if (objectSummary.getStorageClass() != null &&
        ...
        if (objectSummary.getSize() >= S3_MAX_DIRECT_UPLOAD_BYTES) {
          s3Client.copyObjectWithTransferManager(copyRequest);
        } else {
