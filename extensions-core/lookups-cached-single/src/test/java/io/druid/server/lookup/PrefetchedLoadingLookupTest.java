...
public class PrefetchedLoadingLookupTest
{
  PrefetchableFetcher dataFetcher = EasyMock.createMock(PrefetchableFetcher.class);
  ...
  LoadingLookup loadingLookup = new PrefetchedLoadingLookup(dataFetcher, lookupCache, reverseLookupCache);
  ...
  public void testApplyWithExecutionError() throws ExecutionException
  {
    ...
    EasyMock.expect(dataFetcher.prefetch(EasyMock.eq("key"))).andReturn(ImmutableMap.of()).once();
    lookupCache.putAll(EasyMock.anyObject(Map.class));
