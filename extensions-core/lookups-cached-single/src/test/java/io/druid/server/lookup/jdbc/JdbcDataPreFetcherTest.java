...
public class JdbcDataPreFetcherTest
{
  @Rule
  public final TestDerbyConnector.DerbyConnectorRule derbyConnectorRule = new TestDerbyConnector.DerbyConnectorRule();
  Handle handle;

  private JdbcDataPreFetcher jdbcDataPreFetcher;
  ...
  private List<String> rangeList = ImmutableList.of("A", "Z", "a", "e", "z");

  @Before
  public void setUp() throws InterruptedException
  {
    jdbcDataPreFetcher = new JdbcDataPreFetcher(derbyConnectorRule.getMetadataConnectorConfig(), "tableName", "keyColumn", "valueColumn",
        100, rangeList);
  ...
  {
    Assert.assertEquals("null check", null, jdbcDataPreFetcher.fetch("baz"));
    assertMapLookup(lookupMap, jdbcDataPreFetcher);
  }
  ...
  public void testFetchAll()
  {
    ...
    for (Map.Entry<String, String> entry: jdbcDataPreFetcher.fetchAll()
        ) {
  ...
  {
    ImmutableMap.Builder<String,String> mapBuilder = ImmutableMap.builder();
    for (Map.Entry<String, String> entry: jdbcDataPreFetcher.fetch(lookupMap.keySet())
        ) {
      mapBuilder.put(entry.getKey(), entry.getValue());
    }
  ...
  public void testPrefetch()
  {
    ...
    Assert.assertEquals(expected, jdbcDataPreFetcher.prefetch("foo"));
    assertMapLookup(lookupMap, jdbcDataPreFetcher);
  }
  ...
  public void testReverseFetch() throws InterruptedException
  {
    Assert.assertEquals(
        ...
        Sets.newHashSet(jdbcDataPreFetcher.reverseFetchKeys("bar"))
    );
    Assert.assertEquals(
        "reverse lookup should match",
        Sets.newHashSet("how about that"),
        Sets.newHashSet(jdbcDataPreFetcher.reverseFetchKeys("foo"))
    );
    Assert.assertEquals(
        "reverse lookup should match",
        Sets.newHashSet("empty string"),
        Sets.newHashSet(jdbcDataPreFetcher.reverseFetchKeys(""))
    );
    Assert.assertEquals(
        "reverse lookup of none existing value should be empty list",
        Collections.EMPTY_LIST,
        jdbcDataPreFetcher.reverseFetchKeys("does't exist")
    );
  }
  ...
  {
    JdbcDataPreFetcher jdbcDataPreFetcher = new JdbcDataPreFetcher(new MetadataStorageConnectorConfig(), "table", "keyColumn", "ValueColumn",
        100, rangeList);
    DefaultObjectMapper mapper = new DefaultObjectMapper();
    String jdbcDataPreFetcherSer = mapper.writeValueAsString(jdbcDataPreFetcher);
    Assert.assertEquals(jdbcDataPreFetcher, mapper.reader(DataFetcher.class).readValue(jdbcDataPreFetcherSer));
  }
