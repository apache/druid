...
/**
 * remove oss data segment from oss
 */
public class OssDataSegmentKiller implements DataSegmentKiller {
    ...
    public void kill(DataSegment segment) throws SegmentLoadingException {
        ...
        String descPath = OssUtils.descriptorPathForSegmentPath(key);

        //  remove data file
        if (ossClient.doesObjectExist(bucket, key)) {
            log.info("removing index file[%s/%s] from oss", bucket, key);
