...
//    Assert.assertEquals(KinesisIndexTask.Status.NOT_STARTED, client.getStatus(TEST_ID));
...
//  }
//
//  @Test(expected = KinesisIndexTaskClient.TaskNotRunnableException.class)
...
//  {
//    KinesisIndexTask.Status status = KinesisIndexTask.Status.READING;
//
//    Capture<Request> captured = Capture.newInstance();
//    expect(responseHolder.getStatus()).andReturn(HttpResponseStatus.OK);
...
//    replayAll();
//
//    KinesisIndexTask.Status results = client.getStatus(TEST_ID);
//    verifyAll();
//
//    Request request = captured.getValue();
//    Assert.assertEquals(HttpMethod.GET, request.getMethod());
...
//
//    List<URL> expectedUrls = Lists.newArrayList();
//    List<ListenableFuture<KinesisIndexTask.Status>> futures = Lists.newArrayList();
//    for (int i = 0; i < numRequests; i++) {
...
//    }
//
//    List<KinesisIndexTask.Status> responses = Futures.allAsList(futures).get();
//
//    verifyAll();
//    List<Request> requests = captured.getValues();
//
...
//      Assert.assertTrue("unexpectedURL", expectedUrls.contains(requests.get(i).getUrl()));
//      Assert.assertEquals(KinesisIndexTask.Status.READING, responses.get(i));
//    }
//  }
//
//  @Test
...
//  {
//    public TestableKinesisIndexTaskClient(
