...
import org.apache.spark.sql.sources.{Filter, GreaterThanOrEqual, LessThan, LessThanOrEqual}
...
trait InputPartitionReaderBehaviors[T] extends DruidDataSourceV2TestUtils with Matchers { self: AnyFunSuite =>
  def inputPartitionReader( // scalastyle:ignore method.length (Need to wrap the tests in a method to call as one)
                            name: String,
                            conf: () => Broadcast[SerializableHadoopConfiguration],
    ...
    test(s"$name should read the specified segment") {
      ...
      val partitionReader =
        inputPartitionReaderConstructor(firstSegmentString, schema, None, columnTypes, conf(), false, false)
    ...
    test(s"$name should apply filters to string columns") {
      ...
      val partitionReader =
        inputPartitionReaderConstructor(secondSegmentString, schema, filter, columnTypes, conf(), false, false)
    ...
    test(s"$name should handle multi-valued dimensions") {
      ...
      val partitionReader =
        inputPartitionReaderConstructor(thirdSegmentString, schema, None, columnTypes, conf(), false, false)
    ...
    test(s"$name should handle missing columns using default values for nulls") {
      ...
      val partitionReader =
        inputPartitionReaderConstructor(thirdSegmentString, extendedSchema, None, columnTypes, conf(), false, true)
    ...
    test(s"$name should handle missing columns using SQL-compatible null handling") {
      ...
      val partitionReader =
        inputPartitionReaderConstructor(thirdSegmentString, extendedSchema, None, columnTypes, conf(), false, false)
