...
{
  private static final Map<String, String> PARSE_SPEC_TYPE = ImmutableMap.of(
  ...
  private static final Map<String, String> INPUT_FORMAT_TYPE = ImmutableMap.of(
      ParquetExtensionsModule.PARQUET_AVRO_INPUT_PARSER_TYPE,
  ...
  private static final Map<String, Class<? extends InputFormat>> INPUT_FORMAT_CLASS = ImmutableMap.of(
      ParquetExtensionsModule.PARQUET_AVRO_INPUT_PARSER_TYPE,
      ...
      throws IOException
  {
    ...
    String transformed;
    if (withParseType) {
      transformed = StringUtils.format(template, INPUT_FORMAT_TYPE.get(type), type, PARSE_SPEC_TYPE.get(type));
    } else {
      transformed = StringUtils.format(template, INPUT_FORMAT_TYPE.get(type), type);
    }
  ...
  static Object getFirstRow(Job job, String parserType, String parquetPath) throws IOException, InterruptedException
  {
    ...
    InputFormat inputFormat = ReflectionUtils.newInstance(
        INPUT_FORMAT_CLASS.get(parserType),
