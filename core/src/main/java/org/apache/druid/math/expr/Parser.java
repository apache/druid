...
public class Parser
{
   ...
   * @return
   */
  public static Expr applyUnappliedBindings(Expr expr, Expr.ExprInputBindingAnalysis exprInputBindingAnalysis, List<String> bindingsToApply)
  {
    ...
    List<String> unappliedBindingsInExpression = bindingsToApply.stream()
                                     .filter(x -> exprInputBindingAnalysis.getRequiredBindings().contains(x))
          ...
          if (childExpr instanceof ApplyFunctionExpr) {
            ...
            for (Expr arg : fnExpr.args) {
              if (arg.getIdentifierIfIdentifier() == null && arrayInputs.contains(arg)) {
                Expr newArg = applyUnappliedBindings(arg, exprInputBindingAnalysis, unappliedBindingsInExpression);
                newArgs.add(newArg);
              } else {
                newArgs.add(arg);
              }
    ...
    Expr.ExprInputBindingAnalysis newExprBindings = newExpr.analyzeInputs();
  ...
  private static ApplyFunctionExpr liftApplyLambda(ApplyFunctionExpr expr, List<String> unappliedArgs)
  {
    ...
    Set<String> unappliedInThisApply =
        unappliedArgs.stream()
                     .filter(u -> !expr.exprInputBindingAnalysis.getArrayBindings().contains(u))
                     .collect(Collectors.toSet());

    List<String> unappliedIdentifiers =
        expr.exprInputBindingAnalysis
            .getFreeVariables()
            .stream()
    ...
    for (int i = 0; i < expr.argsExpr.size(); i++) {
      newArgs.add(
          applyUnappliedBindings(
              expr.argsExpr.get(i),
              expr.argsExprInputBindingAnalysis.get(i),
    ...
    List<IdentifierExpr> unappliedLambdaBindings =
        expr.lambdaExprInputBindingAnalysis.getFreeVariables()
                                           .stream()
   ...
   */
  public static void validateExpr(Expr expression, Expr.ExprInputBindingAnalysis exprInputBindingAnalysis)
  {
    final Set<String> conflicted =
        Sets.intersection(exprInputBindingAnalysis.getScalarBindings(), exprInputBindingAnalysis.getArrayBindings());
