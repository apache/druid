...
public class DoubleCompressionBenchmarkFileGenerator
{
  ...
  private static String dirPath = "doubleCompress/";
  ...
  public static void main(String[] args) throws IOException, URISyntaxException
  {
    ...
    BenchmarkColumnSchema enumeratedSchema = BenchmarkColumnSchema.makeEnumerated("", ValueType.DOUBLE, true, 1, 0d,
    ...
    for (Map.Entry<String, BenchmarkColumnValueGenerator> entry : generators.entrySet()) {
      ...
      try (Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(dataFile)))) {
        for (int i = 0; i < ROW_NUM; i++) {
          writer.write((Double) entry.getValue().generateRowValue() + "\n");
        }
      }
    }

    // create compressed files using all combinations of CompressionStrategy and DoubleEncoding provided
    for (Map.Entry<String, BenchmarkColumnValueGenerator> entry : generators.entrySet()) {
      for (CompressedObjectStrategy.CompressionStrategy compression : compressions) {
        ...
        DoubleSupplierSerializer writer = CompressionFactory.getDoubleSerializer(
            iopeon,
            "double",
        ...
        try (FileChannel output = FileChannel.open(
          ...
          while ((line = br.readLine()) != null) {
            writer.add(Double.parseDouble(line));
          }
