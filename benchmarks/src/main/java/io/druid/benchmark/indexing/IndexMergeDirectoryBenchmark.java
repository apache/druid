...
public class IndexMergeDirectoryBenchmark
{
  @Param({"5"})
  ...
  private static final Logger log = new Logger(IndexMergeDirectoryBenchmark.class);
  ...
  @BenchmarkMode(Mode.AverageTime)
  @OutputTimeUnit(TimeUnit.MICROSECONDS)
  public void mergeV9Directory(Blackhole blackhole) throws Exception
  {
    File tmpFile = File.createTempFile("IndexMergeBenchmark-MERGEDFILE-V9-" + System.currentTimeMillis(), ".TEMPFILE");
    tmpFile.delete();
    tmpFile.mkdirs();

    Directory directory = new MMapDirectory(tmpFile.toPath());
    log.info(tmpFile.getAbsolutePath() + " isFile: " + tmpFile.isFile() + " isDir:" + tmpFile.isDirectory());
    tmpFile.deleteOnExit();

   INDEX_MERGER_V9.mergeQueryableIndexV1(indexesToMerge, rollup, schemaInfo.getAggsArray(), directory, new IndexSpec());

    blackhole.consume(directory);
