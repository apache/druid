...
public class IndexGeneratorCombinerTest
{
  ...
  public void testMultipleRowsMerged() throws Exception
  {
    ...
    List<BytesWritable> rows = Lists.newArrayList(
        new BytesWritable(InputRowSerde.toBytes(typeHelperMap, row1, aggregators, true).lhs),
        new BytesWritable(InputRowSerde.toBytes(typeHelperMap, row2, aggregators, true).lhs)
    );
